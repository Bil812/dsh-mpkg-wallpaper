...
::selection {
	background-color: color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) 45%, transparent) !important;
...
/* ⑰(修正) 设置面板容器（VOzbGW_panel，settings 组件）：背景用主题抬升色，
   不再是字体色（brand 系 token 被覆盖成 ink 导致） */
[class*="settingsArea"] [class*="panel"],
[class*="settingsArea"] .VOzbGW_panel {
	background-color: color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) 96%, var(--dsw-static-neutral-bluish-850)) !important;
	color: var(--mpw-ink, var(--dsw-alias-label-primary)) !important;
}
