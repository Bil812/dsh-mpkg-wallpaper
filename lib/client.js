	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 壁纸 blur 归谁管：
				//  - 统一虚化开 + chatFollow 开 → unifyAmount 接管（磨砂条禁用，整屏统一）
				//  - 统一虚化开 + chatFollow 关 → 磨砂条控制壁纸 blur，统一虚化只管侧边栏/标题栏白雾
				//  - 统一虚化关 → 磨砂条控制
		...
		function applyTokenOverrides(ctx, section) {
			...
			// 主画布背景（聊天区）：
			//  - 统一虚化开 + chatFollow 开 → unifyAmount 映射的 alpha（0=全透明透出壁纸）
			//  - 统一虚化开 + chatFollow 关 → 磨砂条映射（blur*2.5：0→0% / 40→100%），
			//    统一虚化只控制侧边栏/标题栏白雾（buildCss 的 U()），聊天区归磨砂条
			//  - 统一虚化关 → 0.62（原默认，跟随 dialogBlur）
			const chatFollowTok = section.chatFollow !== void 0 ? !!section.chatFollow : DEFAULT_CHAT_FOLLOW;
			const blurValTok = section.blur !== void 0 ? section.blur : DEFAULT_BLUR;
			const baseAlphaRaw = enabled && hasImage && dialogBlur
				? (unifyTint
					? (chatFollowTok
						...
						: Math.max(0, Math.min(1, blurValTok * 0.025)))
					: 0.62)
				: 1;
		...
		function buildCss(section) {
			...
			// ①(修正) 统一虚化 = 壁纸 blur + 表面白雾**同时**由 unifyAmount 控制：
			// 0 = 壁纸清晰 + 表面全透明（完全透出壁纸）；40 = 强模糊 + 表面实心（朦胧）。
			// 这样「整屏虚化程度」条能同时调节侧边栏/标题栏/聊天区的虚化感，
			// 而不是只改壁纸 blur（上一版）或只改透明度（更早版本）。
			const uAlpha = Math.max(0, Math.min(100, Math.round((unifyAmount / 40) * 100)));
			// 表面白雾厚度：unify 开 → uAlpha（跟随整屏虚化条）；关 → 各区域原值
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 统一虚化开 → 标题栏雾底跟随整屏虚化条（uAlpha，与侧边栏/聊天区一致）；
				// 关 → 由磨砂程度条控制。
...
`;
		...
		function MpkgSection(props) {
			...
			return h("div", { className: "mpw_row" }, [
				...
				// 关 = 磨砂条恢复可调（聊天区壁纸由磨砂条控制），统一虚化只管侧边栏/标题栏白雾
				h("div", { style: { display: (section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT) ? "" : "none" } },
		...
		const zh = {
			...
			"unifyTint": "统一虚化",
			"unifyTint.desc": "开启后，整个界面（侧边栏/标题栏/聊天区等）的虚化感统一由「整屏虚化程度」条控制：0 = 壁纸清晰、界面全透明透出壁纸；拉高 = 壁纸越来越模糊、表面白雾越来越厚（朦胧）。关闭后各区域单独调节",
			"unifyAmount": "整屏虚化程度",
			"chatFollow": "聊天区跟随整屏虚化",
			"chatFollow.desc": "统一虚化开启时：开 = 聊天区壁纸也随「整屏虚化程度」（磨砂条被接管禁用）；关 = 磨砂条恢复可调，聊天区壁纸由磨砂条控制，统一虚化只控制侧边栏/标题栏的白雾",
			"sessionFollow": "新会话按钮跟随整屏虚化",
		...
		const en = {
			...
			"unifyTint.desc": "On: the whole interface (sidebar, title bar, chat area, etc.) shares one blur feel controlled by the Full-screen blur degree slider: 0 = sharp wallpaper with fully transparent surfaces; higher = wallpaper more blurred and surface fog thicker (hazy). Off = per-area control",
			"unifyAmount": "Full-screen blur degree",
			"chatFollow": "Chat follows full-screen blur",
			"chatFollow.desc": "When unified blur is on: On = the chat area wallpaper follows the Full-screen blur degree (frosted slider taken over & disabled); Off = the frosted slider becomes adjustable again, the chat area wallpaper follows it, and unified blur only controls the sidebar/title-bar fog",
			"sessionFollow": "New-chat button follows full-screen blur",
			"sessionFollow.desc": "When unified blur is on: whether the New chat button follows the full-screen blur degree. Off = it follows the panel opacity",
