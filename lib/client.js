	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 壁纸 blur 归谁管（毛玻璃=壁纸层blur+表面半透明，无 backdrop-filter）：
				//  - 统一虚化开 → unifyAmount 接管壁纸 blur（整屏毛玻璃，侧边栏/标题栏
				//    sidebarAlpha 半透明透出；聊天区同理）
				//  - 统一虚化关 → 磨砂条控制
				...
				const bgBlur = unifyTintApply ? unifyAmt : frostBlur;
		...
		function buildCss(section) {
			...
			// ①(修正) 统一虚化 = 侧边栏/标题栏毛玻璃：用 ::before 伪元素做**整块** backdrop blur
			//（像聊天框虚化那样只模糊背后内容，壁纸其他地方清晰）。
			// 用 ::before 而非 sidebarCol/header 本体：本体加 backdrop-filter 会困住
			// fixed 设置弹窗/子代理面板（历史实测 bug）；::before 是兄弟级背景层，不成为
			// 那些 fixed 后代的 containing block → 弹窗正常、且整块模糊无矩形缝隙。
			// 表面白雾厚度由独立条 sidebarAlpha 控制（与 blur 解耦，透明度可单独调）。
			// 0 = 不虚化（清晰）；40 = 强虚化（毛玻璃）。
			const uAlpha = Math.max(0, Math.min(100, (section.sidebarAlpha !== void 0 ? section.sidebarAlpha : DEFAULT_SIDEBAR_ALPHA)));
			// 表面白雾厚度：unify 开 → sidebarAlpha（独立条）；关 → 各区域原值
			const U = (v) => unifyTint ? uAlpha : v;
			// 区域 backdrop-filter：unify 开 → 侧边栏/标题栏用 unifyAmount 的 blur；
			// 关 → 跟随磨砂条。
			const blurFilter = unifyTint ? "none" : (blur > 0 ? `blur(${blur}px)` : "none");
			// ①(修正) 侧边栏/标题栏局部毛玻璃（最终方案 E，严格实测）：
			//  - 模糊层用 sidebarCol/header 的 ::before，z-index:-1（内容之下）、
			//    pointer-events:none（不挡点击）。内容容器 hHd-Xa_root / header 直接子元素
			//    z-index:1 提到模糊层之上 → 图标/按钮/文字清晰可见。
			//  - ::before 不是设置弹窗（fixed）的祖先 → 不困弹窗（实测 w=800 正常）。
			//  - 不加 isolation:isolate（会困 stacking context）。display:contents 中间层
			//    是障碍 → 用真实内容容器 hHd-Xa_root 提升（实测生效）。
			//  - 只模糊侧边栏/标题栏背后（局部），壁纸其他地方保持清晰 —— 满足
			//    「不修改整张壁纸、只改遮挡部分」的需求；适配悬浮（圆角跟随）。
			// ①(修正) 毛玻璃最终安全方案（多轮实测后定型）：
			//  - 给 sidebarCol/header 本体、::before、或 header 子元素提升 z-index 都会引发
			//    新 bug：本体/::before 的 backdrop-filter 困住设置弹窗（fixed）；header 子元素
			//    z-index:1 会盖在设置弹窗（z-index:1）上面 → 设置界面显示聊天内容（用户实测）。
			//  - 安全做法：不给任何侧边栏/标题栏元素加 backdrop-filter、不改 z-index。
			//    毛玻璃 = **壁纸层 filter blur（--mpw-bg-blur）+ 表面半透明透出**
			//    （sidebarAlpha 控制白雾）。整块连续、无缝隙、不困弹窗、图标清晰、
			//    对第三方插件零影响。这是唯一不引入新 bug 的方案。
			const frostedLayers = "";
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(重做) 统一虚化开 → 毛玻璃由 header::before 整块提供（无缝隙）；
				// 雾底跟随侧边栏/标题栏透明度条（sidebarAlpha）；关闭时由磨砂程度条控制标题栏雾底。
				const hblurAmt = section.headerBlurAmount !== void 0 ? section.headerBlurAmount : DEFAULT_HEADER_BLUR_AMOUNT;
				// ①(修正) 统一虚化开 → 标题栏雾底跟随 sidebarAlpha 条；关 → 磨砂程度条。
