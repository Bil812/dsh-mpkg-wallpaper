window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_OPACITY = 55; // ⑰(重做) 面板不透明度默认 55%（0=完全露出壁纸，100=纯遮罩色）
		...
		const DEFAULT_PANEL_MATCH = true; // ⑰(新) 面板颜色自动匹配壁纸主色（开=取壁纸主色，关=仅主题深/浅中性色）
		const DEFAULT_PANEL_COLOR = "";   // ⑰(新) 自定义遮罩/主题色（#rrggbb；空=自动：跟随壁纸或主题中性色）
		const DEFAULT_PANEL_BOOST = 22;   // ⑰(新) 面板加重：全局遮罩之上再叠一层 mask 色的强度（0-50%）
		const DEFAULT_FONT_COLOR = "";    // ⑰(新) 自定义字体颜色（#rrggbb；空=自动按遮罩色生成衍生色）
		const DEFAULT_FONT_COLOR_GRAY = ""; // ⑰(新) 自定义灰色字颜色（#rrggbb；空=自动衍生）
		...
		const DEFAULT_SIDEBAR_ALPHA = 35; // ①(新) 统一虚化下侧边栏/标题栏白雾厚度 0-100（默认 35：顶栏/侧边栏一致且白底淡；用户实测 55 太厚）
		...
		function loadSection() {
			try {
				...
				// ⑰(重做) 一次性迁移：旧版坏滑条会把 opacity 卡在 100（全屏遮罩=纯色盖住壁纸），
				// 且滑块范围已改为 0-100、默认 55 → 自动把它拉回默认值，避免刷新后仍"全白"。
				if ((parsed.opacity === 100 || parsed.opacity === "100") && parsed.opacityMigrated !== true) {
					parsed.opacity = DEFAULT_OPACITY;
					parsed.opacityMigrated = true;
		...
		// ═══════════════════════════════════════════════════════════════════
		//  统一面板遮罩（⑰(重做)）：一个全屏、位于内容底层的半透明覆盖层。
		//  颜色 = 壁纸主色（自动）/ 自定义色 / 主题中性色；透明度 = 面板不透明度。
		//  所有表面在「透出壁纸」开启时一律透明 → 全界面雾色完全一致，
		//  其他遮罩（弹窗/菜单）z-index 更高，天然叠在这层之上。
		//  文字颜色（ink）由遮罩颜色亮度自适应（亮底深字 / 暗底浅字）。
		// ═══════════════════════════════════════════════════════════════════
		let maskEl = null;
		let tintHooked = new WeakSet();
		let lastTintSig = "";
		function ensureMaskEl() {
			if (maskEl && maskEl.isConnected) return maskEl;
			maskEl = document.createElement("div");
			maskEl.id = "mpw-mask";
			maskEl.style.cssText = "position:fixed;inset:0;z-index:-1;pointer-events:none;";
		...
		/** Office 式衍生色面板：母色 → 4 档变亮 + 母色 + 4 档变暗（同色相明暗层级）。 */
		function shadePalette(rgb) {
			const list = [];
			for (const t of [0.85, 0.7, 0.55, 0.4]) list.push(mixRgb(rgb, [255, 255, 255], t));
			list.push(rgb);
			for (const t of [0.3, 0.5, 0.7, 0.85]) list.push(mixRgb(rgb, [0, 0, 0], t));
			return list.map(hexOf);
		}
		/** 自适应文字色：暗遮罩 → 亮字 / 亮遮罩 → 深字（固定高可读中性色，不再用低对比灰）。
		 *  设置了自定义字体色时：primary=自定义色，secondary/tertiary 向对比方向小幅衍生，
		 *  保证次级/三级字也清晰可读。
		 *  ⑰(新) fontGrayRaw：自定义"灰色字"颜色——设置后次级/三级文字（说明、提示、
		 *  caption、dimmed 等灰色层级）直接用该色，不再自动衍生。 */
		function inkForRgb(rgb, darkTheme, fontColorRaw, fontGrayRaw) {
			const er = rgb[0], eg = rgb[1], eb = rgb[2];
			const luma = (0.299 * er + 0.587 * eg + 0.114 * eb) / 255;
			const darkBg = luma <= 0.6;
			const custom = fontColorRaw && /^#[0-9a-fA-F]{3,8}$/.test(fontColorRaw) ? parseHexColor(fontColorRaw) : null;
			const gray = fontGrayRaw && /^#[0-9a-fA-F]{3,8}$/.test(fontGrayRaw) ? parseHexColor(fontGrayRaw) : null;
			if (custom || gray) {
				const toward = darkBg ? [255, 255, 255] : [0, 0, 0];
				// ⑰(修正) 主文字色 ink：有 custom 用 custom；否则（仅设 gray）保持自动中性主色
				// —— 之前 hexOf(null) 崩溃 → refreshMask 异常 → 所有 --mpw-ink* 未设置 →
				// 依赖 var(--mpw-ink-*, fallback) 的元素回落到 DSH 原灰（用户实测"部分灰没变"）
				const inkRgb = custom
					? custom
					: (darkBg ? [238, 241, 247] : [16, 20, 31]);
				const sec = gray
					? hexOf(gray)
					: hexOf(mixRgb(custom, toward, darkBg ? 0.28 : 0.3));
				const ter = gray
					? hexOf(gray)
					: hexOf(mixRgb(custom, toward, darkBg ? 0.48 : 0.5));
				return {
					luma,
					ink: hexOf(inkRgb),
					inkSecondary: sec,
			...
			return darkBg
				? { luma, ink: "#eef1f7", inkSecondary: "#c7d0dd", inkTertiary: "#b3bfcf" }
				: { luma, ink: "#10141f", inkSecondary: "#3d4657", inkTertiary: "#47536a" };
		}
		/** 遮罩有效色：自定义色 > 壁纸主色 + 主题中性色（亮主题少掺壁纸色，保持浅色可读）> 主题中性色。 */
		function effectiveMaskColor(s) {
			const darkTheme = !!(document.body && document.body.hasAttribute("data-ds-dark-theme"));
			const custom = s && s.panelColor && /^#[0-9a-fA-F]{3,8}$/.test(s.panelColor) ? parseHexColor(s.panelColor) : null;
			if (custom) return custom;
			const panelMatch = s && s.panelMatch !== void 0 ? !!s.panelMatch : DEFAULT_PANEL_MATCH;
			if (panelMatch) {
				let tintRaw = "";
				try { tintRaw = getComputedStyle(document.documentElement).getPropertyValue("--mpw-panel-tint").trim(); } catch {}
				const parts = String(tintRaw || "").trim().split(/\s+/).map(Number);
				if (parts.length >= 3 && !parts.some((v) => isNaN(v))) {
					const neutral = darkTheme ? [14, 20, 32] : [255, 255, 255];
					// ⑰(修正) 亮主题只掺 35% 壁纸色 → 深色壁纸也不会把面板拉成深灰；
					// 暗主题掺 55% → 保持暗色玻璃感
					const ratio = darkTheme ? 0.55 : 0.35;
					return [
		...
		/** 刷新全屏遮罩层 + 文字色 token（壁纸取色/主题/设置变化后调用）。 */
		function refreshMask() {
			try {
				...
				if (!enabled || !s.image) {
					...
					root.style.removeProperty("--mpw-mask-rgb");
					root.style.removeProperty("--mpw-surface-rgb");
					root.style.removeProperty("--mpw-ink");
					root.style.removeProperty("--mpw-ink-secondary");
					root.style.removeProperty("--mpw-ink-tertiary");
					lastTintSig = "";
					try { applyTokenOverrides(pluginCtx, readSection()); } catch {}
					return;
				}
				const opacity = s.opacity !== void 0 ? s.opacity : DEFAULT_OPACITY;
				const alpha = Math.max(0, Math.min(100, opacity)) / 100;
				const rgb = effectiveMaskColor(s);
				root.style.setProperty("--mpw-mask-rgb", `${rgb[0]} ${rgb[1]} ${rgb[2]}`);
				const info = inkForRgb(rgb, darkTheme, s && s.fontColor, s && s.fontColorGray);
				// ⑰(新) 浮层抬升色：暗遮罩→提亮 14%，亮遮罩→加深 6%，避免下拉/菜单纯黑或纯白
				const surfaceRgb = info.luma <= 0.6 ? mixRgb(rgb, [255, 255, 255], 0.14) : mixRgb(rgb, [0, 0, 0], 0.06);
				root.style.setProperty("--mpw-surface-rgb", `${surfaceRgb[0]} ${surfaceRgb[1]} ${surfaceRgb[2]}`);
				root.style.setProperty("--mpw-ink", info.ink);
				root.style.setProperty("--mpw-ink-secondary", info.inkSecondary);
				root.style.setProperty("--mpw-ink-tertiary", info.inkTertiary);
				el.style.background = `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${alpha})`;
				el.style.display = "";
				try { applyTokenOverrides(pluginCtx, readSection()); } catch {}
			} catch {}
		}
		/** 从当前壁纸抽主色（48×48 平均色），写入 --mpw-panel-tint 后刷新遮罩。 */
		function applyPanelTint() {
			try {
				...
				const sig = `${R},${G},${B}`;
				if (sig === lastTintSig) return; // 主色未变 → 不重复刷新
				lastTintSig = sig;
				document.documentElement.style.setProperty("--mpw-panel-tint", `${R} ${G} ${B}`);
				refreshMask(); // 自动模式会用新主色重算遮罩 + 文字色
			} catch { /* 跨域/解码失败 → 保持主题中性色 */ }
		}
		function schedulePanelTint() {
			try {
				const { img, video } = bgElements();
				if (!img && !video) return;
				const doTint = () => { try { applyPanelTint(); } catch {} };
				if (img && !tintHooked.has(img)) {
					tintHooked.add(img);
					img.addEventListener("load", doTint, { passive: true });
				}
				if (video && !tintHooked.has(video)) {
					tintHooked.add(video);
					video.addEventListener("loadeddata", doTint, { passive: true });
					// 视频/GIF 动图播放中周期性刷新取色（节流 2s）
					let last = 0;
		...
		function applyFromStorage() {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				const q = image.slice(5); // "host:" 是 5 字符
				// ④(新) 本地壁纸库 URL（ltoken=）走 /library-media；mpkg 条目走 /media；
				// ①(新) 自定义目录（custom=）走 /custom-media
				let hostUrl;
				if (q.indexOf("ltoken=") >= 0) hostUrl = HOST_BASE + "/library-media" + q;
				else if (q.indexOf("custom=") >= 0) hostUrl = HOST_BASE + "/custom-media" + q.replace("custom=1&", "");
				else hostUrl = HOST_BASE + "/media" + q;
			...
			// ⑰(重做) 统一遮罩：有背景 → 挂取色监听 + 刷新全屏遮罩；无背景 → refreshMask 隐藏并清理
			try {
				if (section.image) schedulePanelTint();
				refreshMask();
			} catch {}
		 ...
		 *  ⑰(fork 整合) 支持自定义字体色 fontColor + 自定义灰色字 fontColorGray：
		 *  设置后 primary 用 fontColor、灰字（secondary/tertiary）用 fontColorGray。 */
		function aquaInkForRgb(rgb, section) {
			const er = rgb[0], eg = rgb[1], eb = rgb[2];
			const luma = (0.299 * er + 0.587 * eg + 0.114 * eb) / 255;
			const darkBg = luma <= 0.6;
			const s = section || {};
			const custom = s.fontColor && /^#[0-9a-fA-F]{3,8}$/.test(s.fontColor) ? aquaParseHex(s.fontColor) : null;
			const gray = s.fontColorGray && /^#[0-9a-fA-F]{3,8}$/.test(s.fontColorGray) ? aquaParseHex(s.fontColorGray) : null;
			if (custom || gray) {
				const toward = darkBg ? [255, 255, 255] : [0, 0, 0];
				const inkRgb = custom ? custom : (darkBg ? [238, 241, 247] : [16, 20, 31]);
				const sec = gray ? aquaHexOf(gray) : aquaHexOf(aquaMixRgb(custom, toward, darkBg ? 0.28 : 0.3));
				const ter = gray ? aquaHexOf(gray) : aquaHexOf(aquaMixRgb(custom, toward, darkBg ? 0.48 : 0.5));
				return { luma, ink: aquaHexOf(inkRgb), inkSecondary: sec, inkTertiary: ter };
			}
			return darkBg
				? { luma, ink: "#eef1f7", inkSecondary: "#c7d0dd", inkTertiary: "#b3bfcf" }
				: { luma, ink: "#10141f", inkSecondary: "#3d4657", inkTertiary: "#47536a" };
		 ...
		 *  ⑰(fork 整合) aquaColor 或 panelColor（旧字段）都认，作为自定义遮罩色。 */
		function aquaEffectiveColor(section) {
			const darkTheme = !!(document.body && document.body.hasAttribute("data-ds-dark-theme"));
			const custom = (section.aquaColor && /^#[0-9a-fA-F]{3,8}$/.test(section.aquaColor))
				? aquaParseHex(section.aquaColor)
				: (section.panelColor && /^#[0-9a-fA-F]{3,8}$/.test(section.panelColor) ? aquaParseHex(section.panelColor) : null);
			if (custom) return custom;
		...
		function refreshAqua() {
			try {
				...
				const info = aquaInkForRgb(rgb, s);
		...
		function aquaTokenOverrides(section) {
			...
			const rgb = parts;
			const info = aquaInkForRgb(rgb, section);
		...
		function applyTokenOverrides(ctx, section) {
			...
			// ⑰(重做) 面板不透明度是主控：0=完全露出壁纸，100=纯遮罩色
			const unifyTint = section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT;
			const panelVal = Math.max(0, Math.min(100, opacity)) / 100;
			...
			if (enabled && hasImage && dialogBlur && bdOk) {
				a = Math.min(0.78, 0.60 + 0.15 * (1 - panelVal));
			}
			const toAlpha = (staticVar, alpha) => `color-mix(in srgb, var(${staticVar}) ${Math.round(alpha * 100)}%, transparent)`;
			// ⑰(重做) 统一遮罩色：token 半透明色统一取自 --mpw-mask-rgb（壁纸自动/自定义/主题中性）
			const tinted = (alpha) => `color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) ${Math.round(alpha * 100)}%, transparent)`;
			// ⑰(新) 文字颜色自适应：遮罩色亮 → 深字；暗 → 浅字（中间地带跟随主题）
			let inkOverrides = null;
			let brandOverrides = null; // ⑰(新) 品牌蓝根源：icon/提交按钮/插件文字共用 --dsw-alias-brand-primary
			{
				const darkTheme = !!(document.body && document.body.hasAttribute("data-ds-dark-theme"));
				let maskRaw = "";
				try { maskRaw = getComputedStyle(document.documentElement).getPropertyValue("--mpw-mask-rgb").trim(); } catch {}
				const parts = String(maskRaw || "").trim().split(/\s+/).map(Number);
				if (parts.length >= 3 && !parts.some((v) => isNaN(v))) {
					const info = inkForRgb(parts, darkTheme, section.fontColor, section.fontColorGray);
					inkOverrides = {
						"--dsw-alias-label-primary": { light: info.ink, dark: info.ink },
						"--dsw-alias-label-secondary": { light: info.inkSecondary, dark: info.inkSecondary },
						"--dsw-alias-label-tertiary": { light: info.inkTertiary, dark: info.inkTertiary },
						"--dsw-alias-label-caption": { light: info.inkTertiary, dark: info.inkTertiary },
						"--dsw-alias-label-dimmed": { light: info.inkTertiary, dark: info.inkTertiary },
						// ⑰(修正) 补齐 DSH 其余灰色文字 token（用户实测部分灰字没跟随）：
						// label-primary-bluish/dimmed/foreground 是主文字偏蓝灰/降级灰/前景灰
						"--dsw-alias-label-primary-bluish": { light: info.ink, dark: info.ink },
						"--dsw-alias-label-primary-dimmed": { light: info.inkTertiary, dark: info.inkTertiary },
						"--dsw-alias-label-primary-foreground": { light: info.ink, dark: info.ink },
						// quaternary：第四级灰字（composer 禁用座位/文件链接下划线）
						"--dsw-alias-label-quaternary": { light: info.inkTertiary, dark: info.inkTertiary },
						// inverse / primary-inverted：反色文字（深色按钮上的浅字等）
						"--dsw-alias-label-inverse": { light: info.inkSecondary, dark: info.inkSecondary },
						"--dsw-alias-label-primary-inverted": { light: info.inkSecondary, dark: info.inkSecondary },
						// error：错误文字 → 保持可读（用次级灰；错误场景本身有图标/背景区分）
						"--dsw-alias-label-error": { light: info.inkSecondary, dark: info.inkSecondary },
						// line-secondary / separator-primary / fill：分隔线/次级填充灰
						"--dsw-alias-line-secondary": { light: info.inkTertiary, dark: info.inkTertiary },
						"--dsw-alias-separator-primary": { light: info.inkTertiary, dark: info.inkTertiary },
						"--dsw-alias-fill-l2": { light: tinted(0.75), dark: tinted(0.75) },
						"--dsw-alias-fill-tsp-secondary": { light: tinted(0.65), dark: tinted(0.65) },
						"--dsw-alias-border-secondary": { light: info.inkTertiary, dark: info.inkTertiary },
						// warn-label：警告文字色（黄/橙系），保持可读 → 用次级灰
						"--dsw-alias-state-warn-label": { light: info.inkSecondary, dark: info.inkSecondary }
					};
					brandOverrides = {
						"--dsw-alias-brand-primary": { light: info.ink, dark: info.ink },
						"--dsw-alias-brand-text": { light: info.ink, dark: info.ink },
						"--dsw-alias-brand-primary-invert": { light: info.inkSecondary, dark: info.inkSecondary },
						"--dsw-alias-brand-primary-new-colorprimary-new-color": { light: info.inkSecondary, dark: info.inkSecondary },
						"--dsw-alias-state-business-primary": { light: info.inkSecondary, dark: info.inkSecondary },
						"--dsw-alias-state-business-tertiary": { light: tinted(0.3), dark: tinted(0.3) },
						"--dsw-alias-button-info-fill": { light: tinted(0.82), dark: tinted(0.82) },
						"--dsw-alias-button-info-hover": { light: tinted(0.9), dark: tinted(0.9) }
					};
				}
			}
			// 主画布背景（聊天区）白雾：
			//  - 面板不透明度条主控（panelVal = opacity/100，0=全透明透出壁纸）；
			//  - chatFollow 开 → 聊天区壁纸模糊跟随整屏虚化（磨砂条作废）；
			...
			const overrides = Object.assign({
				...
				// 透明度 0.62：壁纸可见且文字可读。
				"--dsw-alias-bg-base": {
					light: tinted(baseAlpha),
					dark: tinted(baseAlpha)
				},
				...
				// ⑰(新) 白色面板底色全面接管：卡片/弹层/导航/气泡/按钮/提示等全部用 mask 衍生色
				"--dsw-specific-input-major": { light: tinted(0.9), dark: tinted(0.9) },
				"--dsw-specific-selector": { light: tinted(0.78), dark: tinted(0.78) },
				"--dsw-specific-menu": { light: tinted(0.86), dark: tinted(0.86) },
				"--dsw-specific-tip": { light: tinted(0.85), dark: tinted(0.85) },
				"--dsw-specific-login-input": { light: tinted(0.85), dark: tinted(0.85) },
				"--dsw-specific-sidebar-fill": { light: tinted(0.85), dark: tinted(0.85) },
				"--dsw-specific-sidebar-nav-item-active-accent": { light: tinted(0.6), dark: tinted(0.6) },
				"--dsw-specific-bubble": { light: tinted(0.84), dark: tinted(0.84) },
				"--dsw-specific-bubble-highlight": { light: tinted(0.92), dark: tinted(0.92) },
				"--dsw-specific-sidebar-nav-item-active": { light: tinted(0.72), dark: tinted(0.72) },
				"--dsw-specific-sidebar-nav-item-hover": { light: tinted(0.6), dark: tinted(0.6) },
				"--dsw-alias-bg-layer-3": { light: tinted(a), dark: tinted(a) },
				"--dsw-alias-bg-module-platform": { light: tinted(0.86), dark: tinted(0.86) },
				"--dsw-alias-bg-multi-select": { light: tinted(0.82), dark: tinted(0.82) },
				"--dsw-alias-button-elevated-fill": { light: tinted(0.78), dark: tinted(0.78) },
				"--dsw-alias-button-floating-fill": { light: tinted(0.85), dark: tinted(0.85) },
				...
				"--dsw-alias-interactive-bg-hover": { light: tinted(0.55), dark: tinted(0.55) },
				"--dsw-alias-interactive-bg-hover-solid": { light: tinted(0.72), dark: tinted(0.72) },
				"--dsw-alias-tooltip-bg": { light: `color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) 96%, var(--dsw-static-neutral-bluish-850))`, dark: `color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) 96%, var(--dsw-static-neutral-bluish-850))` },
				"--dsw-alias-toast-bg": { light: tinted(0.9), dark: tinted(0.9) },
				"--dsw-alias-markdown-inline-code": { light: tinted(0.24), dark: tinted(0.24) },
				"--dsw-alias-markdown-code-block": { light: tinted(0.32), dark: tinted(0.32) },
				"--dsw-alias-markdown-code-block-banner": { light: tinted(0.36), dark: tinted(0.36) },
				"--dsw-alias-markdown-citation": { light: tinted(0.24), dark: tinted(0.24) }
			}, Object.assign({}, inkOverrides || {}, brandOverrides || {}));
			if (!(enabled && hasImage)) {
		...
		function applyDialogInline(section) {
			...
			const floatOn = section.float !== void 0 ? !!section.float : DEFAULT_FLOAT;
			const panel = Math.max(0, Math.min(100, section.opacity !== void 0 ? section.opacity : DEFAULT_OPACITY));
		...
		function buildCss(section) {
			...
			const thinkBg = section.thinkBg !== void 0 ? !!section.thinkBg : DEFAULT_THINK_BG;
			const panel = Math.max(0, Math.min(100, opacity));
			const details = Math.min(100, panel + 3);
			const hideBg = hasImage && panel >= 100;
			// ⑰(修正) 面板不透明度 = 所有区域白雾厚度（唯一主控）。
			// 之前 unifyTint 开时各区域 alpha 被 unifyAmount 接管：拖动不透明度
			// 中间值毫无变化、只有到 100% 才因 hideBg 让壁纸消失 → 观感"只有 0/100%"。
			// 现在 U() 恒等于面板值；「统一虚化」滑条改为驱动整屏 backdrop 模糊。
			const unifyTint = section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT;
			...
			// ⑰(新) 面板加重：在全局遮罩之上再叠一层 mask 色，让面板比纯背景更醒目
			//（必须先定义：sidebarFog/headerFog 的默认值引用 panelBoost，后置会触发 TDZ 崩溃）
			const panelBoost = section.panelBoost !== void 0 ? section.panelBoost : DEFAULT_PANEL_BOOST;
			// ⑰(新) 三区独立虚化：侧边栏/标题栏各自雾底可单独调（默认跟随当前行为）
			// sidebarFog：侧边栏 mask 雾底 %（0=全透出壁纸，100=实心）；默认用面板加重值
			// headerFog：标题栏 mask 雾底 %；默认也用面板加重值（保持原观感，不透明）
			//（fogOf："" 或非法值视为未设置 → 用默认值，避免恢复默认后雾底变 0 透明）
			const fogOf = (v, def) => (v !== void 0 && v !== "" && !Number.isNaN(Number(v))) ? Math.max(0, Math.min(100, Number(v))) : def;
			const sidebarFog = fogOf(section.sidebarFog, panelBoost);
			const headerFog = fogOf(section.headerFog, panelBoost);
			// ⑰(改) 分区虚化（用户要"虚化大小"而非雾底）：侧边栏/标题栏各自 backdrop blur px
			//（0 = 不输出局部 blur → 跟随整屏虚化；>0 = 该区域独立虚化半径，优先于整屏）
			const pxOf = (v) => { const n = v !== void 0 && v !== "" ? Number(v) : NaN; return Number.isNaN(n) ? 0 : Math.max(0, Math.min(40, Math.round(n))); };
			const sidebarPx = pxOf(section.sidebarBlurPx);
			const headerPx = pxOf(section.headerBlurPx);
			const chatFollow = section.chatFollow !== void 0 ? !!section.chatFollow : DEFAULT_CHAT_FOLLOW;
			// chatFollow：聊天区壁纸是否随面板不透明度；关 → 由磨砂模糊条接管
			//（磨砂条越高聊天区白雾越厚：alpha = blur*2.5，0→0% / 40→100%）。
			const chatAlpha = unifyTint
				? (chatFollow ? panel : Math.max(0, Math.min(100, Math.round(blur * 2.5))))
				: 0; // 非 unify 时不用（聊天区走 token 0.62）
			const U = (v) => Math.max(0, Math.min(100, v));
			// ⑰(重做) 遮罩由全屏 #mpw-mask 覆盖层提供（颜色/透明度由 JS 实时计算），
			// 这里所有表面在「透出壁纸」开启时统一透明 → 全界面雾色一致。
			// 新会话按钮：unify 开时随整屏虚化（sessionFollow），否则随面板不透明度
			const sessionAlpha = unifyTint
				? ((section.sessionFollow !== void 0 ? !!section.sessionFollow : DEFAULT_SESSION_FOLLOW) ? uAlpha : panel)
				: panel;
			// 区域 backdrop-filter：unify 开 → 整屏模糊感由 unifyAmount 条提供；
			// 关 → 跟随磨砂条（壁纸层模糊仍由 --mpw-bg-blur 提供）。
			const blurFilter = unifyTint
				? (unifyAmount > 0 ? `blur(${unifyAmount}px)` : "none")
				: (blur > 0 ? `blur(${blur}px)` : "none");
			//（panelBoost 定义已上移至 sidebarFog/headerFog 之前，避免 TDZ）
			const boostBg = (extra) => `color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) ${Math.max(0, Math.min(100, Math.round(extra)))}%, transparent)`;
			// ⑰(新) 浮层抬升色背景（暗遮罩提亮/亮遮罩加深后的 --mpw-surface-rgb）
			const surfaceBg = (extra) => `color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) ${Math.max(0, Math.min(100, Math.round(extra)))}%, transparent)`;
			const newSessionBg = boostBg(Math.min(100, sessionAlpha + 15));
			...
			// ⑰(修正) 灰色文字 token 直接写入 CSS（不依赖 overrideTokens 机制，确保生效）：
			// 所有 label 灰色层级 → inkTertiary（自定义灰色字），主文字 → ink。
			// **无条件生成**：不依赖 hasImage / --mpw-mask-rgb（host 壁纸或未启用时
			// 这些可能缺失 → 之前覆盖块不生成，灰字永远不跟随）。
			// mask 色缺失时用主题中性色兜底（暗 #0e1420 / 亮 #f5f6f7），字体色仍生效。
			try {
				const darkTheme = !!(document.body && document.body.hasAttribute("data-ds-dark-theme"));
				let maskRaw = "";
				try { maskRaw = getComputedStyle(document.documentElement).getPropertyValue("--mpw-mask-rgb").trim(); } catch {}
				const parts = String(maskRaw || "").trim().split(/\s+/).map(Number);
				const fallbackRgb = darkTheme ? [14, 20, 32] : [245, 246, 247];
				const maskRgb = (parts.length >= 3 && !parts.some((v) => isNaN(v))) ? parts : fallbackRgb;
				const info = inkForRgb(maskRgb, darkTheme, section.fontColor, section.fontColorGray);
				css += `
body:not([data-mpw-aqua]),
body:not([data-mpw-aqua]) * {
	--dsw-alias-label-primary: ${info.ink} !important;
	--dsw-alias-label-secondary: ${info.inkSecondary} !important;
	--dsw-alias-label-tertiary: ${info.inkTertiary} !important;
	--dsw-alias-label-caption: ${info.inkTertiary} !important;
	--dsw-alias-label-dimmed: ${info.inkTertiary} !important;
	--dsw-alias-label-primary-bluish: ${info.ink} !important;
	--dsw-alias-label-primary-dimmed: ${info.inkTertiary} !important;
	--dsw-alias-label-primary-foreground: ${info.ink} !important;
	--dsw-alias-label-quaternary: ${info.inkTertiary} !important;
	--dsw-alias-label-inverse: ${info.inkSecondary} !important;
	--dsw-alias-label-primary-inverted: ${info.inkSecondary} !important;
	--dsw-alias-label-error: ${info.inkSecondary} !important;
	--dsw-alias-line-secondary: ${info.inkTertiary} !important;
	--dsw-alias-separator-primary: ${info.inkTertiary} !important;
	--dsw-alias-border-secondary: ${info.inkTertiary} !important;
	--dsw-alias-state-warn-label: ${info.inkSecondary} !important;
}
/* ⑰(诊断) 直接覆盖 body 级变量（与 DSH 同选择器、后注入 → 必赢） */
body:not([data-mpw-aqua]) {
	--dsw-alias-label-primary: ${info.ink} !important;
	--dsw-alias-label-secondary: ${info.inkSecondary} !important;
	--dsw-alias-label-tertiary: ${info.inkTertiary} !important;
...
/* ⑰(重做) 侧边栏根：全局遮罩之上再叠一层 mask 色（sidebarFog 独立可调），比纯背景更醒目 */
.pI_x6G_sidebarCol,
.hHd-Xa_root {
	background-color: ${boostBg(sidebarFog)} !important;
}
/* ⑰(加固) 兜底：hash 类名变化时用 data-slot 再保证一次（侧边栏不再 100% 不透明） */
[data-slot="sidebar"] {
	background-color: ${boostBg(sidebarFog)} !important;
}
.hHd-Xa_newSession {
	/* ④(新) 统一虚化开：随整屏虚化（sessionFollow）或随面板不透明度；关：随面板不透明度 */
	/* ⑰(新) 背景用 mask 衍生色，文字用自适应 ink（比主题色更亮/暗） */
	background-color: ${newSessionBg} !important;
	color: var(--mpw-ink, var(--dsw-alias-label-primary)) !important;
	border-color: color-mix(in srgb, var(--mpw-ink-secondary, var(--dsw-alias-border-l2)) 55%, transparent) !important;
}
...
.ydkMvW_root {
	/* ⑰(修正) 详情面板（悬浮工作区出现）：用浮层抬升色，不再灰 */
	background-color: ${surfaceBg(92)} !important;
}
...
.hHd-Xa_regionArea {
	/* ⑰(重做) 工作区列表不再单独 blur：与面板统一（壁纸自身磨砂 + 全局遮罩）。
	   不给侧边栏任何容器加 backdrop-filter（会困设置弹窗 / 形成分离模糊块）；
	   毛玻璃 = 壁纸层 blur + 表面半透明（见 frostedLayers 注释）。 */
...
`;
			if (headerBlur && headerBg) {
				...
				// ⑰(重做) 标题栏：遮罩之上再叠一层 mask 色（headerFog 独立可调），磨砂程度条只影响视觉雾感。
				...
				css += `
.wSkVaW_header {
	background-color: ${boostBg(headerFog)} !important;
	backdrop-filter: none !important;
...
`;
			...
			// 设置面板/通用对话框是 role=dialog：token override 把 bg-base 变半透明后
...
`;
			if (!confirmFrosted) css += `
/* 下载/确认弹窗虚化关 → 弹窗不透明兜底（防透出壁纸；⑰ 统一用遮罩色实底） */
.mpw_dialog {
	background: rgb(var(--mpw-mask-rgb, 14 20 32)) !important;
}
...
/* 提问输入框（composer）：跟随「虚化对话框」开关（聊天框不随统一虚化）。
   虚化开 = 半透明背景 + backdrop blur（真玻璃效果），透出模糊壁纸/经过的字；
   工具按钮/添加按钮同步用衍生色 + 自适应文字色。 */
[data-composer-card] {
	background-color: ${boostBg(55)} !important;
}
[data-composer-card] [class*="tools"] button,
[data-composer-card] [class*="add"] {
	background-color: color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) 78%, transparent) !important;
	color: var(--mpw-ink, var(--dsw-alias-label-primary)) !important;
	border: none !important;
}
[data-composer-card] [class*="tools"] button:hover:not(:disabled),
[data-composer-card] [class*="add"]:hover:not(:disabled) {
	background-color: color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) 92%, transparent) !important;
}
...
`;
			...
			// ── F. 统一虚化 + 聊天区不跟随：侧边栏/标题栏背后跟随整屏虚化度 ──
			// chatFollow 关 → 壁纸层 blur = 磨砂条（frostBlur）供聊天区；
			// 侧边栏/标题栏再叠 backdrop blur(unifyAmt)，调整「整屏虚化程度」即可改变
			// 侧边栏/标题栏背后的虚化（用户实测：当前关了 chatFollow 后 unifyAmount 失效）。
			...
			if (unifyTint && !chatFollowInCss && unifyAmount > 0 && bdSupported) {
				css += `
/* ── 统一虚化 + 聊天区不跟随：侧边栏/标题栏背后叠加整屏虚化度 ── */
html body:not([data-mpw-sblur-off]) .pI_x6G_sidebarCol,
html body:not([data-mpw-sblur-off]) [class*="sidebarCol"],
html body:not([data-mpw-sblur-off]) [class*="wSkVaW_header"] {
	backdrop-filter: blur(${unifyAmount}px) !important;
	-webkit-backdrop-filter: blur(${unifyAmount}px) !important;
}
`;
			}
			// ⑰(改) 分区虚化（用户要求"虚化大小"独立可调，非雾底）：
			// 侧边栏/标题栏各自 backdrop blur px；0=不输出（跟随整屏虚化），>0=独立半径。
			// 沿用弹窗摘除（body:not([data-mpw-sblur-off])），避免 backdrop-filter 困住 fixed 弹窗。
			// 特异性 (0,2,2) 高于 E 块侧边栏磨砂与 F 块整屏跟随，输出在后 → 优先。
			if ((sidebarPx > 0 || headerPx > 0) && bdSupported) {
				if (sidebarPx > 0) css += `
/* ── 分区虚化：侧边栏独立 blur ── */
html body:not([data-mpw-sblur-off]) .pI_x6G_sidebarCol,
html body:not([data-mpw-sblur-off]) [class*="sidebarCol"] {
	backdrop-filter: blur(${sidebarPx}px) !important;
	-webkit-backdrop-filter: blur(${sidebarPx}px) !important;
}
`;
				if (headerPx > 0) css += `
/* ── 分区虚化：标题栏独立 blur ── */
html body:not([data-mpw-sblur-off]) [class*="wSkVaW_header"] {
	backdrop-filter: blur(${headerPx}px) !important;
	-webkit-backdrop-filter: blur(${headerPx}px) !important;
}
`;
			}
...
`;
			...
			// ⑯ 虚化关 → 输入框半透明玻璃（用户要求玻璃效果；保留轻微底色保证可读）
...
[data-cordis-panel] {
	...
	background-color: ${boostBg(panelBoost)} !important;
}
[data-cordis-panel] [class*="row"] {
	background-color: transparent !important;
}
/* ⑰(新) 行内代码颜色 = 主题色衍生色（secondary，随遮罩/自定义字体色变化）；
   pre 内语法高亮的代码保持原有配色不动 */
code:not(pre code) {
	color: var(--mpw-ink-secondary, inherit) !important;
}
/* 品牌 Logo 矩形：随自适应 ink（不再用主题品牌蓝/白） */
.hHd-Xa_brand svg rect {
	fill: var(--mpw-ink, currentColor) !important;
}
/* ⑰(修正) 光标/选区/焦点描边颜色：跟随文字色衍生（不再用品牌蓝） */
* { caret-color: var(--mpw-ink-secondary, currentColor) !important; }
::selection {
	/* ⑰(修正) 选中高亮用文字色 26%：浅底 → 深色高亮、深底 → 浅色高亮，
	   与面板/壁纸背景形成对比（之前 mask 色 45% 和浅粉背景融为一体看不见） */
	background-color: color-mix(in srgb, var(--mpw-ink, #888) 26%, transparent) !important;
	color: var(--mpw-ink, inherit) !important;
}
:focus-visible { outline-color: var(--mpw-ink-secondary, currentColor) !important; }
/* ⑰(修正) 复制悬浮卡片（_card_1b2ny_13，硬编码 #2C2C2E 背景 + #fff 已复制白字）：
   背景改主题抬升色、所有文字改自定义灰色字（inkTertiary，跟随 fontColorGray）——
   否则亮色主题下白字白底看不见、内容文字还是原色 */
[aria-label^="复制"],
[aria-label^="Copy"] {
	--dsw-hovercard-bg: color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) 96%, var(--dsw-static-neutral-bluish-850)) !important;
	background: var(--dsw-hovercard-bg) !important;
	color: var(--mpw-ink-tertiary, var(--mpw-ink, #eef1f7)) !important;
}
/* 卡片内部内容文字（常态显示被 hover 的文本，有自己的颜色）+ "已复制"（硬编码 #fff）：
   全部强制为自定义灰色字 */
[aria-label^="复制"] *,
[aria-label^="Copy"] * {
	color: var(--mpw-ink-tertiary, var(--mpw-ink, #eef1f7)) !important;
}
/* ⑰(修正) 会话 hover 卡片（侧边栏会话悬停预览）：标题/路径/时间/状态 硬编码
   #fff/#cfd3d6/#adb2b8 —— 全部改为自适应灰色字（跟随 fontColorGray） */
[class*="hoverContent"] [class*="hoverPath"],
[class*="hoverContent"] [class*="hoverTime"],
[class*="hoverContent"] [class*="hoverStatus"],
[class*="hoverContent"] [class*="hoverTitle"] {
	color: var(--mpw-ink-tertiary, var(--mpw-ink, #eef1f7)) !important;
}
/* ⑰(修正) 选中文字的提示条（selection toolbar）：背景用抬升色 + 边框 + 阴影，
   与下方面板/壁纸区分开（之前 tooltip-bg = mask 色 → 和背景融合看不见） */
[role="tooltip"],
[data-dsh-surface] [role="tooltip"],
[aria-label*="selected"],
[aria-label*="选择"] {
	background-color: color-mix(in srgb, rgb(var(--mpw-surface-rgb, var(--mpw-mask-rgb, 14 20 32))) 97%, var(--dsw-static-neutral-bluish-850)) !important;
	color: var(--mpw-ink, var(--dsw-alias-label-primary)) !important;
	border: 1px solid color-mix(in srgb, var(--mpw-ink-secondary, var(--dsw-alias-border-l2)) 35%, transparent) !important;
	box-shadow: 0 6px 20px rgba(0, 0, 0, 0.18) !important;
}
/* ⑰(修正) 设置面板容器（VOzbGW_panel，settings 组件）：
   透明毛玻璃 —— 半透明遮罩色透出（已模糊的）壁纸 + 自适应文字色。
   之前显示字体色（brand 系 token 被覆盖成 ink）→ 这里显式用遮罩色；
   只精准命中 .VOzbGW_panel（不用 [class*="panel"] 泛化，避免染到面板内部元素） */
[class*="settingsArea"] .VOzbGW_panel,
[class*="settingsArea"] [class*="settingsPanel"] {
	background-color: color-mix(in srgb, rgb(var(--mpw-mask-rgb, 14 20 32)) 55%, transparent) !important;
	color: var(--mpw-ink, var(--dsw-alias-label-primary)) !important;
	backdrop-filter: none !important;
}
...
body[data-mpw-aqua] {
	...
	--dsw-alias-label-primary-bluish: var(--mpw-aqua-ink, inherit);
	--dsw-alias-label-primary-dimmed: var(--mpw-aqua-ink-tertiary, inherit);
	--dsw-alias-label-primary-foreground: var(--mpw-aqua-ink, inherit);
	--dsw-alias-label-quaternary: var(--mpw-aqua-ink-tertiary, inherit);
	--dsw-alias-label-inverse: var(--mpw-aqua-ink-secondary, inherit);
	--dsw-alias-label-primary-inverted: var(--mpw-aqua-ink-secondary, inherit);
	--dsw-alias-label-error: var(--mpw-aqua-ink-secondary, inherit);
	--dsw-alias-line-secondary: var(--mpw-aqua-ink-tertiary, inherit);
	--dsw-alias-separator-primary: var(--mpw-aqua-ink-tertiary, inherit);
	--dsw-alias-border-secondary: var(--mpw-aqua-ink-tertiary, inherit);
	--dsw-alias-state-warn-label: var(--mpw-aqua-ink-secondary, inherit);
...
/* ⑰(fork 整合) Aqua 模式下：会话 hover 卡片硬编码灰字（#cfd3d6/#adb2b8）跟随灰字色 */
body[data-mpw-aqua] [class*="hoverContent"] [class*="hoverPath"],
body[data-mpw-aqua] [class*="hoverContent"] [class*="hoverTime"],
body[data-mpw-aqua] [class*="hoverContent"] [class*="hoverStatus"],
body[data-mpw-aqua] [class*="hoverContent"] [class*="hoverTitle"] {
	color: var(--mpw-aqua-ink-tertiary, inherit) !important;
}
...
.mpw_sectionBtn {
	...
	border: none; outline: none; font: inherit; letter-spacing: 0.02em;
}
.mpw_sectionBtn:hover { background: color-mix(in srgb, var(--dsw-alias-label-primary) 12%, transparent); }
.mpw_sectionBtn:focus-visible { outline: 2px solid var(--dsw-alias-brand-primary, #7aa2ff); outline-offset: 1px; }
.mpw_sectionBtn.mpw_collapsed { color: var(--dsw-alias-label-secondary); }
/* ⑳(移植官方 v3.1.3) 设置页顶部 Tab 书签导航：圆角导航栏 + 下划线平滑移动 + 内容真滑动 */
.mpw_tabBar {
...
/* ⑰(新) Office 式调色板：母色衍生色 swatch */
.mpw_palette { display: flex; flex-wrap: wrap; gap: 6px; }
.mpw_swatch {
	flex: none; width: 22px; height: 22px; padding: 0; border-radius: 6px;
	border: 1px solid rgba(0,0,0,.28); cursor: pointer; box-sizing: border-box;
}
.mpw_swatch:hover { transform: scale(1.1); }
.mpw_swatch.mpw_on { outline: 2px solid var(--dsw-alias-brand-primary, #3964fe); outline-offset: 1px; }
/* ⑳(移植官方 v3.1.3) 取色盘色块（点开 HSV 取色器的按钮） */
.mpw_colorSwatch {
...
/* ⑳(移植官方 v3.1.3) 取色盘预置色：elysia395 风格圆（加大、白边、阴影、hover 放大） */
.mpw_presetSwatch {
...
/* ⑰(新) 自定义遮罩/主题色 */
.mpw_colorInput {
	flex: none; width: 44px; height: 30px; padding: 0; box-sizing: border-box;
	border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px;
	background: transparent; cursor: pointer;
}
.mpw_colorInput::-webkit-color-swatch-wrapper { padding: 2px; }
.mpw_colorInput::-webkit-color-swatch { border: none; border-radius: 6px; }
...
.mpw_dialog {
	...
	/* ①(修正) 弹窗背景不透明：layer-2 token 会被本插件的虚化 override 半透明化，
	   导致弹窗透出壁纸；⑰(重做) 改用统一遮罩色实底，与自适应文字色一致。 */
	background: rgb(var(--mpw-mask-rgb, 14 20 32));
...
`;
		...
		function MpkgSection(props) {
			...
			// ⑰(新) 设置分组折叠：界面虚化/透出壁纸/其他 默认收起，避免设置页过长
			const [collapsed, setCollapsed] = react.useState({ blur: true, show: true, other: true });
			...
			const scanLibrary = async () => {
				...
				try {
					...
					// 只并入能实际播放的 video（scene 只有预览图、web 未实现），
					// 用 src:"steam" 标记，next/prev 走 applyLibraryWallpaper。
					const steamPlayable = walls.filter((wp) => wp.type === "video" && wp.media);
					if (steamPlayable.length) {
						setWallList((prev) => {
							const merged = steamPlayable.map((wp) => ({
								key: "steam|" + wp.ltoken, title: wp.title || wp.ltoken,
								name: wp.title || wp.ltoken, type: "video", src: "steam",
								ltoken: wp.ltoken, media: wp.media
							}));
							const existing = prev || [];
			...
			const scanCustomDir = async (dirArg) => {
				...
				try {
					...
					const d = await r.json();
					if (!d.ok) { showError(t("lib.dirFail") + String(d.error || "")); }
					else {
						setCustomFiles(d.files || []);
						// ①(新) 合并进统一列表：自定义目录文件（图片+视频）
						setWallList((d.files || []).map((f) => ({
			...
			const applyCustomMpkg = async (w) => {
				setLibBusy(true);
				try {
					const r = await fetch(HOST_BASE + "/custom-mpkg?file=" + encodeURIComponent(w.name));
					const d = await r.json();
					if (!d.ok || !d.selected) { showError(t("mpkg.noAsset") + "（" + w.name + "）"); setLibBusy(false); return; }
					const sel = d.selected;
					commit({
						image: "host:?token=" + d.token + "&index=" + sel.index + (sel.offset ? "&offset=" + sel.offset : ""),
						source: sel.name, mpkgKey: "custommpkg|" + w.name, mpkgName: w.name,
					...
					setMpkgMeta({ name: w.name, key: "custommpkg|" + w.name, info: null, entryName: sel.name, slot: null });
					setHint(t("lib.applied") + "：" + w.name);
			...
			const applyWallFromList = (w) => {
				...
				// ②(新) Steam 库项 → 走 applyLibraryWallpaper（video 专用）
				if (w.src === "steam") {
					applyLibraryWallpaper({ type: "video", ltoken: w.ltoken, title: w.title, media: w.media });
					return;
				}
				commit({
			...
			const nextWallpaper = () => {
				...
				const idx = (wallIdx + 1) % list.length;
				setWallIdx(idx);
				applyWallFromList(list[idx]);
			};
			// 1(新) 切换到上一个壁纸
			const prevWallpaper = () => {
				const list = wallList;
				if (!list.length) { showError(t("lib.empty")); return; }
				const idx = (wallIdx - 1 + list.length) % list.length;
				setWallIdx(idx);
				applyWallFromList(list[idx]);
			};
			// ③(新) 壁纸轮换：定时自动切换
			const startRotation = () => {
				...
				const sec = section.rotateMin > 0 ? section.rotateMin : 5;
			...
			const applyLibraryWallpaper = (wp) => {
				if (wp.type === "web") { showError(t("lib.webPending")); return; }
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					panelMatch: DEFAULT_PANEL_MATCH,
					panelColor: "",
					panelBoost: DEFAULT_PANEL_BOOST,
					fontColor: "",
					fontColorGray: "",
					sidebarFog: "",
					headerFog: "",
			...
			// ⑰(新) 可折叠分组：点击标题展开/收起
			const group = (key, label, children) => h(react.Fragment, null, [
				h("button", {
					type: "button",
					className: "mpw_section mpw_sectionBtn" + (collapsed[key] ? " mpw_collapsed" : ""),
					onClick: () => setCollapsed(Object.assign({}, collapsed, { [key]: !collapsed[key] }))
				}, (collapsed[key] ? "▸ " : "▾ ") + label),
				collapsed[key] ? null : children
			]);
			// ⑳(移植官方 v3.1.3) HSV 自由取色器（绑定 panelColor/fontColor 等字段）
			let pickerInst = null;
			...
			// ⑰(新) Office 式调色板：母色衍生色 + 自定义取色器 + 恢复自动
			const maskBaseRgb = (() => {
				try {
					const v = getComputedStyle(document.documentElement).getPropertyValue("--mpw-mask-rgb").trim();
					const p = String(v).split(/\s+/).map(Number);
					if (p.length >= 3 && !p.some((n) => isNaN(n))) return [p[0], p[1], p[2]];
				} catch {}
				return [31, 42, 61];
			})();
			const palette = shadePalette(maskBaseRgb);
			const colorRow = (label, desc, field, autoLabel) => h("div", { className: "mpw_field" }, [
				h("label", { className: "mpw_label" }, label),
				// ⑳(移植官方 v3.1.3) 取色器入口（HSV 拖拽）+ 预置色（elysia395 圆）
				h("div", { className: "mpw_inline" }, [
					h("button", {
						className: "mpw_colorSwatch", "data-mpw-picker-anchor": field, type: "button",
						style: { background: (section[field] && /^#[0-9a-fA-F]{6}$/.test(section[field]) ? section[field] : "#808080") },
						onClick: () => openPicker(field, section[field])
					}),
					...(AQUA_PRESETS.map((hex) => h("button", {
						className: "mpw_presetSwatch", type: "button", key: hex, title: hex,
						style: { background: hex }, onClick: () => commit({ [field]: hex }, true)
					}))),
					section[field]
						? h("button", { className: "mpw_reset mpw_miniBtn", type: "button", onClick: () => commit({ [field]: "" }, true) }, autoLabel)
						: null
				]),
				// Office 式衍生色板（母色同色相 4 亮 4 暗 + 母色）
				h("div", { className: "mpw_inline mpw_palette" }, palette.map((c) => {
					const active = String(section[field] || "").toLowerCase() === c;
					return h("button", {
						type: "button",
						key: c,
						className: "mpw_swatch" + (active ? " mpw_on" : ""),
						style: { background: c },
						title: c,
						onClick: () => commit({ [field]: c }, true)
					});
				})),
				h("p", { className: "mpw_hint" }, desc)
			]);
			...
			return h("div", { className: "mpw_row" }, [
				...
				// ⑳(移植官方 v3.1.3) 顶部 Tab 书签导航
				h("div", { className: "mpw_tabBar", "data-mpw-tabbar": "" }, [
					h("div", { className: "mpw_tabIndicator", "data-mpw-tabind": "" }),
					TAB_ORDER.map((tid) => h("button", {
						className: "mpw_tab" + (settingsTab === tid ? " mpw_tabActive" : ""),
						type: "button", key: tid,
						onClick: () => switchTab(tid)
					}, t("sec." + tid)))
				]),
				// 内容区：内层 flex 行 translateX 滑动（所有 tab 始终挂载，hooks 稳定）
				...
				h("div", { className: "mpw_field" }, [
					...
					h("div", { className: "mpw_inline" }, [
						...
						h("button", { className: "mpw_fileBtn", type: "button", onClick: () => {
							commit({ customDirPath: customDir.trim() }, true);
							scanCustomDir();
						} }, t("lib.scanDir"))
					]),
					...
					libOpen && libWalls && libWalls.length
						? h("div", { className: "mpw_props mpw_wallGrid", style: { maxHeight: 220, overflowY: "auto" } }, [
							libWalls.slice(0, libShow).map((wp, wi) => h("div", { className: "mpw_prop mpw_wallProp", key: wp.ltoken }, [
								h("div", { className: "mpw_inline", style: { alignItems: "flex-start", flexWrap: "nowrap" } }, [
									// ①(新) Steam 库预览缩略图（preview.jpg 走 /library-media；视频用 media 首帧）
									h("div", { className: "mpw_thumb" }, [
				...
				sliderRow(t("opacity"), "opacity", 0, 100, "%", 1, DEFAULT_OPACITY),
				...
				// ⑰(改) 分区虚化（用户要"虚化大小"）：侧边栏/标题栏各自 backdrop blur px，
				// 0 = 跟随整屏虚化；>0 = 该区域独立虚化半径（弹窗打开时自动摘除防困弹窗）
				sliderRow(t("sidebarBlurPx"), "sidebarBlurPx", 0, 40, "px", 1, 0),
				sliderRow(t("headerBlurPx"), "headerBlurPx", 0, 40, "px", 1, 0),
				...
				// ⑰(fork 整合) 灰色字自定义色：次级/三级灰字（说明/提示/caption/dimmed 等）
				h("div", { className: "mpw_field" }, [
					h("label", { className: "mpw_label" }, t("fontColorGray")),
					h("div", { className: "mpw_inline" }, [
						h("button", {
							className: "mpw_colorSwatch", "data-mpw-picker-anchor": "fontColorGray", type: "button",
							style: { background: (section.fontColorGray && /^#[0-9a-fA-F]{6}$/.test(section.fontColorGray) ? section.fontColorGray : "#808080") },
							onClick: () => openPicker("fontColorGray", section.fontColorGray)
						}),
						...(AQUA_PRESETS.map((hex) => h("button", {
							className: "mpw_presetSwatch", type: "button", key: "g" + hex, title: hex,
							style: { background: hex }, onClick: () => commit({ fontColorGray: hex }, true)
						}))),
						h("button", { className: "mpw_miniBtn", type: "button", onClick: () => commit({ fontColorGray: "" }, true) }, t("aquaColorReset"))
					]),
					h("p", { className: "mpw_hint" }, t("fontColorGray.desc"))
				]),
				...
				updConfirm ? h("div", { className: "mpw_mask", onClick: () => setUpdConfirm(null) }, [
				...
				// ①(新) 目录选择弹窗（文件夹选择器，跨平台）
				dirPick ? h("div", { className: "mpw_mask", onClick: () => setDirPick(false) }, [
					h("div", { className: "mpw_dialog", onClick: (ev) => ev.stopPropagation() }, [
						...
						h("div", { className: "mpw_props", style: { maxHeight: 240, overflowY: "auto" } }, [
		...
		const zh = {
			...
			"lib.rotateMin": "轮换间隔",
			"lib.minutes": "分钟",
			"hybrid.desc": "开启：mpkg 上传到 DSH 进程流式播放，支持 >600MB 大文件（需重启 dsh web 使宿主端生效）；关闭：纯浏览器模式（600MB 上限）",
			...
			"sec.source": "背景来源",
			"sec.appearance": "外观",
			"sec.appearance.desc": "面板不透明度=全界面统一遮罩（0=完全露出壁纸，100=纯遮罩色，拖动实时生效）；磨砂模糊=整张壁纸的模糊程度（0=清晰）；镜头缩放/位置=背景画面的放大与平移；遮罩颜色可自动跟随壁纸或自定义；面板可加重突出；字体颜色自动生成同色相衍生色或自定义",
			"sec.blur": "界面虚化",
			...
			"opacity": "面板不透明度",
			"panelMatch": "面板颜色跟随壁纸",
			"panelMatch.desc": "开：全界面遮罩自动取壁纸主色（第三方跨域图片无法取色时回退主题色）；关：只用主题深/浅中性色。自定义主题色优先于以上",
			"panelColor": "自定义主题色（遮罩色）",
			"panelColor.desc": "选一个颜色作为全界面遮罩底色（优先于壁纸自动取色）；文字颜色会自动按该色明暗调整",
			"panelColorAuto": "恢复自动",
			"panelBoost": "面板加重",
			"panelBoost.desc": "在全局遮罩之上再叠一层遮罩色，让面板（侧边栏/聊天区/标题栏）比纯背景更醒目",
			"fontColor": "自定义字体颜色",
			"fontColor.desc": "空=按遮罩/主题色自动生成同色相衍生色（亮/暗层级，类似 Office）；选色后 primary 用该色，次级/三级自动衍生",
			"fontColorGray": "自定义灰色字颜色",
			"fontColorGray.desc": "次级/三级灰色文字（说明、提示、caption、dimmed 等）的颜色；空=随上方字体色自动衍生（可读中性灰）",
			"fontColorAuto": "恢复自动",
			"brightness": "画面亮度",
			"float": "悬浮效果",
			...
			"sidebarBlurPx": "侧边栏虚化",
			"sidebarBlurPx.desc": "侧边栏区域壁纸的模糊半径（0 = 跟随整屏虚化，越大越模糊）；独立于「侧边栏磨砂」（磨砂是玻璃开关）",
			"headerBlurPx": "标题栏虚化",
			"headerBlurPx.desc": "标题栏区域壁纸的模糊半径（0 = 跟随整屏虚化，越大越模糊）；弹窗打开时自动摘除防困弹窗",
		...
		const en = {
			...
			"lib.minutes": "min",
			"hybrid.desc": "On: mpkg is uploaded to the DSH host and streamed (supports files >600MB; restart dsh web for the host half). Off: pure browser mode (600MB cap)",
			...
			"sec.appearance.desc": "Panel opacity = the full-UI mask (0 = wallpaper fully visible, 100 = solid mask color; live while dragging); Frosted blur = how blurred the wallpaper itself is (0 = sharp); Lens zoom/position = zoom and pan of the background image; the mask color can auto-follow the wallpaper or be a custom color; panels can get extra emphasis; font colors auto-derive same-hue shades or use a custom color",
			"sec.blur": "UI blur",
			...
			"panelMatch": "Match panels to wallpaper",
			"panelMatch.desc": "On: the full-UI mask auto-picks the wallpaper's dominant color (falls back to theme neutrals for cross-origin images). Off: theme dark/light neutrals only. A custom theme color takes priority over both",
			"panelColor": "Custom theme (mask) color",
			"panelColor.desc": "Pick a color used as the full-UI mask base (takes priority over the wallpaper color); text color adapts to its brightness automatically",
			"panelColorAuto": "Auto",
			"panelBoost": "Panel emphasis",
			"panelBoost.desc": "Extra mask layer on top of the global mask so panels (sidebar/chat/title bar) stand out from the bare background",
			"fontColor": "Custom font color",
			"fontColor.desc": "Empty = auto-derived shades of the mask/theme color (same hue, lighter/darker steps, like Office). When set, primary uses your color and secondary/tertiary derive from it",
			"fontColorGray": "Custom gray text color",
			"fontColorGray.desc": "Color of secondary/tertiary gray text (descriptions, hints, captions, dimmed, etc.); empty = auto-derived from the font color above (readable neutral gray)",
			"fontColorAuto": "Auto",
			...
			"sidebarBlurPx": "Sidebar blur",
			"sidebarBlurPx.desc": "Blur radius of the wallpaper behind the sidebar (0 = follow the full-screen blur, higher = more blurred); independent of Sidebar frost (that is the glass toggle)",
			"headerBlurPx": "Title-bar blur",
			"headerBlurPx.desc": "Blur radius of the wallpaper behind the title bar (0 = follow the full-screen blur, higher = more blurred); auto-lifted while a dialog is open",
		...
		function applyInner(ctx) {
			// 背景 DOM 常驻
			ensureBgDom();
			// ⑰(调试) 输出 body 直接子元素，便于定位第三方浮层/容器（如 body>div:nth-child(7)）
			try {
				console.log("[dsh-mpkg-wallpaper] body children:", Array.from(document.body.children).map((el) => (el.tagName || "") + (el.id ? "#" + el.id : "") + (el.className && typeof el.className === "string" ? "." + String(el.className).slice(0, 60) : "")));
			} catch {}
			...
			applyFromStorage();
			// ⑰(新) 主题切换时刷新遮罩/文字色（深/浅主题的面板有效色与 ink 不同）
			try {
				if (!window.__mpwThemeWatch) {
					window.__mpwThemeWatch = new MutationObserver(() => {
						try { refreshMask(); } catch {}
					});
					window.__mpwThemeWatch.observe(document.body, { attributes: true, attributeFilter: ["data-ds-dark-theme"] });
				}
			} catch {}
