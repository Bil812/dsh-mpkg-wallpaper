	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 壁纸 blur 归谁管：
				//  - 统一虚化开 + chatFollow 开 → unifyAmount 接管（磨砂条禁用，聊天区壁纸也统一模糊）
				//  - 统一虚化开 + chatFollow 关 → 磨砂条控制壁纸 blur；统一虚化只给侧边栏/标题栏
				//    加 backdrop blur（buildCss 的 regionArea/titleRow 规则）
				//  - 统一虚化关 → 磨砂条控制
		...
		function buildCss(section) {
			...
			// ①(修正) 统一虚化 = 给侧边栏/标题栏加**真正的 backdrop blur**（像聊天框虚化那样，
			// 只模糊它们背后的内容，壁纸其他区域保持清晰），强度由 unifyAmount 控制。
			// 表面白雾厚度（透明度）与 blur 解耦：统一跟随「不透明度」条（panel）。
			// 0 = 不虚化（清晰）；40 = 强虚化（毛玻璃）。
...
.hHd-Xa_regionArea {
	/* ①(修正) 统一虚化开 → 真正的 backdrop blur（毛玻璃，像聊天框虚化那样只模糊背后内容）；
	   关 → 跟随磨砂条（壁纸层 blur + 半透明透出）。regionArea 无 fixed 后代，可安全加。 */
	backdrop-filter: ${unifyTint ? `blur(${unifyAmount}px)` : blurFilter} !important;
}
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(重做) 统一虚化开 → 给 header 内部的 titleRow 加真正的 backdrop blur
				//（titleRow 实测无 fixed 后代，安全，像聊天框虚化那样只模糊标题栏背后内容），
				// 雾底跟随「不透明度」条；关闭时由磨砂程度条控制标题栏雾底。
				const hblurAmt = section.headerBlurAmount !== void 0 ? section.headerBlurAmount : DEFAULT_HEADER_BLUR_AMOUNT;
				// ①(修正) 统一虚化开 → 标题栏雾底跟随「不透明度」条（panel）；关 → 磨砂程度条。
...
`;
		...
		const zh = {
			...
			"unifyTint": "统一虚化",
			"unifyTint.desc": "开启后，侧边栏和标题栏获得类似聊天框的毛玻璃虚化（只模糊它们背后的内容，壁纸其他地方保持清晰），强度由「整屏虚化程度」条控制；表面白雾厚度跟随「不透明度」条。关闭后各区域单独调节",
			"unifyAmount": "整屏虚化程度",
		...
		const en = {
			...
			"unifyTint.desc": "On: the sidebar and title bar get a chat-box-like frosted blur (only what's behind them is blurred; the wallpaper stays sharp elsewhere), strength controlled by the Full-screen blur degree slider; surface white-fog thickness follows the Opacity slider. Off = per-area control",
