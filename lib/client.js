	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 壁纸 blur 归谁管（毛玻璃=壁纸层blur+表面半透明，见 frostedLayers 注释）：
				//  - 统一虚化开 + chatFollow 开 → unifyAmount 接管（聊天区壁纸也统一模糊）
				//  - 统一虚化开 + chatFollow 关 → 磨砂条控制壁纸 blur（侧边栏/标题栏靠
				//    sidebarAlpha 半透明透出已模糊壁纸；聊天区壁纸由磨砂条定）
				//  - 统一虚化关 → 磨砂条控制
		...
		function buildCss(section) {
			...
			// ①(修正) 毛玻璃最终方案（对齐 dsh-wallpaper-engine 参考项目）：
			//  - DSH 自带侧边栏（sidebarCol/root/footArea 含设置弹窗 fixed）**不能安全加
			//    backdrop-filter**（困弹窗，实测）；给内部容器（regionArea/logoRow/titleRow
			//    等）分别加则形成分离的模糊块 + 圆角不匹配缝隙（用户实测）。
			//  - 正确做法：不给任何侧边栏/标题栏元素加 backdrop-filter。毛玻璃 =
			//    **壁纸层 filter blur（--mpw-bg-blur，unifyAmount 或磨砂条控制）+
			//    表面半透明透出**（sidebarAlpha 控制白雾）。整块连续、无缝隙、
			//    不困弹窗、图标清晰、对第三方插件零影响。
			const frostedLayers = "";
