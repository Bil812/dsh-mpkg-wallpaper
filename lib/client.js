	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				//    由 frostedLayers 的 ::before 局部提供（壁纸其他地方保持清晰）
				//  - 统一虚化关 → 磨砂条控制
		...
		function buildCss(section) {
			...
			// ①(修正) 侧边栏/标题栏局部毛玻璃（最终方案，全部实测通过）：
			//  - 给**无 fixed 后代**的内容容器加 backdrop blur（局部、不困弹窗）：
			//    regionArea（工作区列表）、logoRow/brand（品牌）、newSession（新会话）、
			//    titleRow/tabs（标题栏）。实测：图标可见、设置弹窗正常（w=800 居中）。
			//  - 绝不给 sidebarCol/hHd-Xa_root/header 本体加 backdrop-filter（会困设置弹窗
			//    fixed 遮罩，实测 w=280 错位）。不加任何 z-index（避免盖弹窗面板）。
			//  - 缝隙（容器间 8px 间距 + footArea 底部）用与模糊区接近的半透明背景
			//    掩盖，视觉连续。适配悬浮（圆角在浮层样式里处理）。
			const frostedLayers = unifyTint ? `
.hHd-Xa_regionArea,
.hHd-Xa_logoRow,
.hHd-Xa_brand,
.hHd-Xa_newSession,
...
/* 缝隙掩盖：容器间距区域补半透明背景，与模糊区同色，视觉连续 */
.hHd-Xa_root {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-950) ${Math.round(uAlpha * 0.8)}%, transparent) !important;
}
body:not([data-ds-dark-theme]) .hHd-Xa_root {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-00) ${Math.round(uAlpha * 0.8)}%, transparent) !important;
}
` : "";
...
/* ③(修正) 弹窗打开时：背后的设置面板只去掉 backdrop-filter（不再让字晕染），
   ...
   ①(修正) 设置弹窗面板必须**不透明**：token override 把 bg-base 变半透明后，
   弹窗面板透出背后的聊天标题/按钮/对话框（用户实测"聊天内容显示在设置界面"）。
   用主题静态色恢复不透明面板（亮=白/暗=深），彻底挡住背后。 */
...
[role="dialog"],
[role="alertdialog"] {
	/* ①(修正) 弹窗面板不透明（防 token 半透明透出聊天内容）：
	   不覆盖 dialogBlur 的 backdrop-filter（那是弹窗自己的虚化），只保证背景不透明 */
	background-color: var(--dsw-static-neutral-bluish-00) !important;
}
