window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			// ①(修正) 毛玻璃模糊层（::before 整块覆盖，无缝隙）：
			const frostedLayers = unifyTint ? `
/* 侧边栏毛玻璃层：覆盖整块 sidebarCol，无矩形缝隙；z-index 在内容之下 */
.pI_x6G_sidebarCol {
	position: relative !important;
}
.pI_x6G_sidebarCol::before {
	content: ""; position: absolute; inset: 0; z-index: 0; pointer-events: none;
	backdrop-filter: blur(${unifyAmount}px) !important;
	-webkit-backdrop-filter: blur(${unifyAmount}px) !important;
}
.pI_x6G_sidebarCol > * { position: relative; z-index: 1; }
/* 标题栏毛玻璃层：覆盖整块 header，无缝隙 */
.wSkVaW_header {
	position: relative !important;
}
.wSkVaW_header::before {
	content: ""; position: absolute; inset: 0; z-index: 0; pointer-events: none;
	backdrop-filter: blur(${unifyAmount}px) !important;
	-webkit-backdrop-filter: blur(${unifyAmount}px) !important;
}
.wSkVaW_header > * { position: relative; z-index: 1; }
` : "";
...
.hHd-Xa_regionArea {
	/* ①(修正) 统一虚化开 → 毛玻璃由 sidebarCol::before 整块提供（regionArea 不再单独 blur，
	   避免矩形缝隙）；关 → 跟随磨砂条（壁纸层 blur + 半透明透出）。 */
