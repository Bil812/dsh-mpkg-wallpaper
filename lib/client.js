...
${floatOn ? `/* ①(修正) 悬浮效果（默认关）：侧边栏/标题栏悬浮卡片。
   ...
   ①(修正) 不再用 margin 缩小 sidebarCol（会把底部时间显示等内容切掉）；
   改为保持原尺寸 + 右侧圆角 + 边框，无阴影——悬浮感靠圆角边框，缝隙不露出白雾。 */
...
/* 2(修正) 参考 DSH-Transparent-UI-Plugin：四周 margin + 全圆角 = 真正的悬浮卡片。
   关键：sidebar root 解除固定宽度（[data-mpw-sidebar-root]{width:100%}）让内容适应
   缩小后的面板，不再溢出被切（之前只右侧圆角 + 左侧贴边看不出悬浮，且切割内容）。 */
...
/* 1(修正) 悬浮时 sidebar root 完全贴合圆角卡片：
   宽度 100%（防刷新后 DSH 还原固定宽度 → 右缘被切）、背景透明（避免"圆角大矩形套直角厚矩形"）、
   圆角跟随（内容显示在圆角矩形内） */
		...
		function MpkgSection(props) {
			...
			return h("div", { className: "mpw_row" }, [
				...
				h("div", { className: "mpw_field" }, [
					...
					wallList.length ? h("div", { className: "mpw_props", style: { maxHeight: 260, overflowY: "auto" } }, [
						wallList.slice(0, libShow).map((w) => h("div", { className: "mpw_prop", key: w.key }, [
							h("div", { className: "mpw_propLabel" }, h("b", null, w.title)),
							h("div", { className: "mpw_inline" }, [
								h("span", { className: "mpw_hint" }, w.type),
								h("button", { className: "mpw_reset mpw_moreBtn", type: "button", onClick: () => applyWallFromList(w) }, t("lib.use"))
							])
						])),
