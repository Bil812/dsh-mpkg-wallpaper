window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 *  保证次级/三级字也清晰可读。 */
		function inkForRgb(rgb, darkTheme, fontColorRaw) {
			...
			if (custom) {
				...
				return {
					luma,
					ink: hexOf(custom),
					inkSecondary: hexOf(mixRgb(custom, toward, darkBg ? 0.18 : 0.2)),
					inkTertiary: hexOf(mixRgb(custom, toward, darkBg ? 0.38 : 0.4))
				};
			}
			return darkBg
		...
		function refreshMask() {
			try {
				...
				const info = inkForRgb(rgb, darkTheme, s && s.fontColor);
		...
		function applyTokenOverrides(ctx, section) {
			...
			let brandOverrides = null; // ⑰(新) 品牌蓝根源：icon/提交按钮/插件文字共用 --dsw-alias-brand-primary
			{
				...
				if (parts.length >= 3 && !parts.some((v) => isNaN(v))) {
					const info = inkForRgb(parts, darkTheme, section.fontColor);
					inkOverrides = {
...
/* 提问输入框（composer）：跟随「虚化对话框」开关（聊天框不随统一虚化）。
   虚化开 = 半透明背景，透出模糊壁纸/经过的字；卡片用 mask 衍生色近实底（不再白色）；
   工具按钮/添加按钮同步用衍生色 + 自适应文字色。 */
[data-composer-card] {
	background-color: ${boostBg(90)} !important;
}
...
/* ⑰(修正) 复制悬浮卡片（_card_1b2ny_13，硬编码 #2C2C2E 深灰）：改为主题抬升色 */
[aria-label^="复制"],
[aria-label^="Copy"] {
	...
	color: var(--mpw-ink, #eef1f7) !important;
}
...
`;
		...
		function MpkgSection(props) {
			...
			const colorRow = (label, desc, field, autoLabel) => h("div", { className: "mpw_field" }, [
				...
				h("div", { className: "mpw_inline" }, [
					h("input", { type: "color", className: "mpw_colorInput", value: section[field] || "#10141f", onChange: (ev) => commit({ [field]: ev.target.value }, true) }),
					section[field]
			...
			return h("div", { className: "mpw_row" }, [
				...
				// ═══ 外观 ═══
				group("appearance", t("sec.appearance"), [
					// ②(新) 外观组说明：各项作用一目了然
				...
				// ⑰(新) 面板颜色自动匹配壁纸（默认开；跨域图片无法取色时自动回退主题色）
				toggleRow(t("panelMatch"), t("panelMatch.desc"), "panelMatch", DEFAULT_PANEL_MATCH),
				// ⑰(新) 自定义遮罩/主题色（Office 式衍生色调色板）
				colorRow(t("panelColor"), t("panelColor.desc"), "panelColor", t("panelColorAuto")),
				// ⑰(新) 面板加重：在全局遮罩之上再叠一层，让面板更醒目
				sliderRow(t("panelBoost"), "panelBoost", 0, 50, "%", 1, DEFAULT_PANEL_BOOST),
				// ⑰(新) 自定义字体颜色（Office 式衍生色调色板）
				colorRow(t("fontColor"), t("fontColor.desc"), "fontColor", t("fontColorAuto")),
