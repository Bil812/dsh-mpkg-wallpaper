...
/* ③(修正) 弹窗打开时：背后的设置面板不再虚化（避免白底黑字晕染看不清）。
   有壁纸时我们的虚化会给 [role=dialog]（设置面板）加 backdrop-filter → 弹窗背后字晕染。 */
body[data-mpw-modal] [role="dialog"] {
	backdrop-filter: none !important;
	-webkit-backdrop-filter: none !important;
	background-color: var(--dsw-static-neutral-bluish-950) !important;
}
body[data-mpw-modal] body:not([data-ds-dark-theme]) [role="dialog"] {
	background-color: var(--dsw-static-neutral-bluish-00) !important;
}
/* ④ 冲突确认弹窗 */
.mpw_mask {
