window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			// ①(修正) 毛玻璃实现（最终方案，踩坑记录）：
			//  - sidebarCol/header 本体、hHd-Xa_root **不能**加 backdrop-filter：设置弹窗
			//    （VOzbGW_overlay, fixed）是它们的后代 → 会被困进 containing block →
			//    设置界面叠到聊天框下面（实测 bug）。isolation:isolate 同理会困进
			//    stacking context；display:contents 中间 div 使 `> * z-index` 失效。
			//  - 安全做法：只给**无 fixed 后代**的内容容器加 backdrop blur ——
			//    regionArea（工作区列表）、logoRow/brand（品牌）、newSession（新会话）、
			//    titleRow/tabs（标题栏）。这些容器实测均无 fixed 后代 → 弹窗正常浮动、
			//    整块连续模糊无缝隙。footArea（设置按钮区）不加，按钮保持清晰。
			//  - 对第三方插件（桌宠等 fixed 层）零影响：不加 isolation、不改全局 z-index。
...
.hHd-Xa_regionArea {
	/* ①(修正) 统一虚化开 → blur 由 frostedLayers 提供（unifyAmount 值），此处被后续规则覆盖；
	   关 → 跟随磨砂条（壁纸层 blur + 半透明透出）。 */
