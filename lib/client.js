...
`;
			...
			// 磨砂对话框是否生效：dialogBlur 开 + 有虚化量 + 浏览器支持 backdrop-filter。
...
/* 磨砂玻璃原理：半透明背景 + backdrop blur 缺一不可。
   ...
   90% 半透明 + blur：背后文字变糊影不可读，壁纸色调微微透出。 */
[role="dialog"],
[role="alertdialog"] {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-00) 90%, transparent) !important;
}
body[data-ds-dark-theme] [role="dialog"],
body[data-ds-dark-theme] [role="alertdialog"] {
	background-color: color-mix(in srgb, var(--dsw-static-neutral-bluish-950) 90%, transparent) !important;
}
...
/* ①(修正) 侧边栏收起态也有悬浮效果：保留圆角卡片 + 边框 + 半透明，
   只是 margin 更小适配窄 rail（之前直接 margin:0/border:none = 完全没有悬浮感） */
...
/* ③(修正) 弹窗打开时：背后的设置面板只去掉 backdrop-filter（不再让字晕染），
   ...
   ①(修正) 弹窗面板不透明（防 token 半透明透出聊天内容）只在 **dialogBlur 关**时生效：
   dialogBlur 开时走磨砂路径（上方虚化块已把背景改为 90% 半透明 + blur），
   这里再覆盖成全不透明会把 blur 完全遮住 → 纯白无模糊（用户实测）。 */
...
[role="dialog"],
[role="alertdialog"] {
	/* ①(修正) 弹窗面板不透明（dialogBlur 关时防 token 半透明透出聊天内容） */
	background-color: var(--dsw-static-neutral-bluish-00) !important;
}
body[data-ds-dark-theme] [role="dialog"],
body[data-ds-dark-theme] [role="alertdialog"] {
