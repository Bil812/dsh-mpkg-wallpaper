window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function handleVideoTexes(mpkg, vtex, file, t, setHint, readEntry) {
			try {
				...
				if (!list.length) {
					...
					setHint(anyBig ? t("mpkg.vtexBig") : t("mpkg.noAsset"));
					return false;
				}
				// 选当前时段
				let active = null;
				...
				setHint(t("mpkg.fail") + String(err && err.message || err));
				return false;
			}
		}
		...
		function applyFromStorage() {
			...
			if (image === "idb:blob") {
				...
				idbGet("bg").then((v) => { if (v && img.src !== v) { img.src = v; showImageEl(); } }).catch(() => {});
			} else {
				lastBgSig = null;
				if (img.src !== image) { img.src = image; showImageEl(); }
			}
			if (image) {
...
`;
		...
		function MpkgSection(props) {
			...
			const onMpkg = async (e) => {
				...
				if (!type) { setHint(t("file.unsafe")); return; }
				...
				if (type !== "mpkg") { setHint(t("file.unsafe")); return; }
				// ⑨(新) 整体大小上限：超大 mpkg（>600MB）移动端浏览器几乎无法处理，直接提示
				if (file.size > 600 * 1024 * 1024) { setHint(t("mpkg.huge")); return; }
				setBusy(true);
				setHint("");
				try {
					...
					if (vtex.length) {
						const ok = await handleVideoTexes(mpkg, vtex, file, t, setHint, readEntry);
						if (ok) {
					...
					if (!entry) { setHint(t("mpkg.noAsset")); return; }
					let stored = null;
					let isVideo = false;
					...
					if (isMp4 && entry.size <= 600 * 1024 * 1024) {
						...
						if (!imgPick || !imgPick.entry) { setHint(t("mpkg.videoHuge")); return; }
