...
`;
		...
		function MpkgSection(props) {
			...
			const [customDir, setCustomDirState] = react.useState("");
				...
				try {
					...
					if (!d.ok) { showError(t("lib.dirFail") + String(d.error || "")); }
						...
						setWallList((d.files || []).map((f) => ({
							key: "custom|" + f.name, title: f.name, type: f.type,
							image: "host:?custom=1&file=" + encodeURIComponent(f.name),
							converted: f.type === "video" ? "mp4" : "gif"
						})));
					}
				} catch (err) { showError(t("lib.fail") + String(err && err.message || err)); }
				setLibBusy(false);
			...
			const applyCustomMpkg = async (w) => {
				...
				try {
					...
					const d = await r.json();
					if (!d.ok || !d.selected) { showError(t("mpkg.noAsset")); setLibBusy(false); return; }
					const sel = d.selected;
					commit({
						image: "host:?token=" + d.token + "&index=" + sel.index,
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					rotate: DEFAULT_ROTATE, rotateMin: 5, customDirPath: "", brightness: DEFAULT_BRIGHTNESS,
			...
			return h("div", { className: "mpw_row" }, [
				...
				h("div", { className: "mpw_field" }, [
					...
					h("div", { className: "mpw_inline" }, [
						h("input", { className: "mpw_input", type: "text", style: { flex: 1, minWidth: 0 },
							defaultValue: section.customDirPath || "",
				...
				h("div", { className: "mpw_field" }, [
					...
					libWalls && libWalls.length
						? h("div", { className: "mpw_props", style: { maxHeight: 220, overflowY: "auto" } }, [
							libWalls.map((wp) => h("div", { className: "mpw_prop", key: wp.ltoken }, [
								h("div", { className: "mpw_propLabel" }, h("b", null, wp.title || wp.ltoken)),
								h("div", { className: "mpw_inline" }, [
									h("span", { className: "mpw_hint" }, wp.type),
									h("button", { className: "mpw_reset mpw_moreBtn", type: "button", onClick: () => applyLibraryWallpaper(wp) }, t("lib.use"))
								])
							]))
						])
				...
				dirPick ? h("div", { className: "mpw_mask", onClick: () => setDirPick(false) }, [
					h("div", { className: "mpw_dialog", onClick: (ev) => ev.stopPropagation() }, [
						...
						h("p", { className: "mpw_hint", style: { wordBreak: "break-all" } }, t("lib.curDir") + "：" + (dirPath || "(根)")),
						h("div", { className: "mpw_props", style: { maxHeight: 240, overflowY: "auto" } }, [
							dirPath ? h("div", { className: "mpw_prop", key: "__up" }, [
								h("button", { className: "mpw_reset mpw_moreBtn", type: "button", onClick: () => {
									const up = dirPath.split(/[\\/]/).filter(Boolean).slice(0, -1).join("/") || (dirPath.includes("\\") ? "C:\\" : "/");
									openDirPicker(up);
								} }, "⬆ " + t("lib.up"))
							]) : null,
		...
		const zh = {
			...
			"update.fail": "检查失败：",
			"update.warnTitle": "新版本刚刚发布",
			"update.warnBody": "该版本发布时间不足 24 小时，可能存在问题。确定要现在更新吗？（版本 ",
			"update.confirmDownload": "确认下载更新",
		...
		const en = {
			...
			"update.warnTitle": "New version just released",
			"update.warnBody": "This version was published less than 24h ago and may have issues. Update anyway? (v",
			"update.confirmDownload": "Confirm download & update",
