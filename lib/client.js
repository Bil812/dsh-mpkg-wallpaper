	...
	factory: (require) => {
		...
		function applyFromStorage() {
			...
			if (image) {
				...
				// ①(修正) 统一虚化开时：壁纸 blur 由 unifyAmount 接管（磨砂条作废），
				// 这样拖动 unifyAmount 才真正改变"虚化程度"（与对话框虚化同语义）；
				// 关闭时壁纸 blur 由磨砂模糊条控制。
				...
				const bgBlur = unifyTintApply ? unifyAmt : (section.blur !== void 0 ? section.blur : DEFAULT_BLUR);
		...
		function buildCss(section) {
			...
			// 表面白雾厚度：unify 开 → 统一用 panel（跟随不透明度条）；关 → 各区域原值
			const U = (v) => unifyTint ? panel : v;
...
.hHd-Xa_newSession {
	...
	background-color: color-mix(in srgb, var(--dsw-alias-button-elevated-fill) ${unifyTint ? (section.sessionFollow !== void 0 ? !!section.sessionFollow : DEFAULT_SESSION_FOLLOW) ? panel : panel : panel}%, transparent) !important;
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 统一虚化开 → 标题栏雾底跟随「不透明度」条（与侧边栏一致）；
				// 关 → 由磨砂程度条控制。
				const hdrAlpha = unifyTint ? panel : Math.max(0, Math.min(100, hblurAmt));
...
`;
		...
		function MpkgSection(props) {
			...
			const resetSettings = () => {
				...
				writeSection(Object.assign({}, keep, {
					...
					unifyTint: DEFAULT_UNIFY_TINT, unifyAmount: DEFAULT_UNIFY_AMOUNT, sessionFollow: DEFAULT_SESSION_FOLLOW,
			...
			return h("div", { className: "mpw_row" }, [
				...
				// ①(修正) 磨砂模糊条：统一虚化开启时被「整屏虚化程度」接管 → 禁用并提示（不再隐藏，避免"这一项不见了"）
				h("div", { style: (section.unifyTint !== void 0 ? !!section.unifyTint : DEFAULT_UNIFY_TINT) ? { opacity: 0.45, pointerEvents: "none" } : {} },
		...
		const zh = {
			...
			"blur": "磨砂模糊",
			"blur.overridden": "统一虚化开启中：壁纸模糊由「整屏虚化程度」接管，磨砂条暂不可调；关闭统一虚化后恢复",
			"zoom": "镜头缩放",
			"lens.pos": "镜头位置（平移）",
			"lens.x": "X",
			"lens.y": "Y",
			...
			"unifyTint": "统一虚化",
			"unifyTint.desc": "开启后，整个界面（侧边栏/标题栏/聊天区等）透出的壁纸模糊程度统一由「整屏虚化程度」条控制：0 = 壁纸清晰，拉高 = 壁纸越来越模糊（虚化度）。表面白雾厚度统一跟随「不透明度」条。关闭后各区域单独调节",
			"unifyAmount": "整屏虚化程度",
			"sessionFollow": "新会话按钮跟随不透明度",
			"sessionFollow.desc": "统一虚化开启时：「添加新会话」按钮是否随「不透明度」条（与侧边栏一致）。关 = 保持原按钮色",
			"dialogBlur": "虚化对话框",
		...
		const en = {
			...
			"blur": "Frosted blur",
			"blur.overridden": "Unified blur is on: wallpaper blur is taken over by the Full-screen blur degree slider, so this frosted slider is disabled; turn unified blur off to restore it",
			...
			"unifyTint.desc": "On: the whole interface (sidebar, title bar, chat area, etc.) shows the same wallpaper blur degree controlled by one slider: 0 = sharp wallpaper, higher = more blurred (blur degree). Surface white-fog thickness follows the Opacity slider. Off = per-area control",
			"unifyAmount": "Full-screen blur degree",
			"sessionFollow": "New-chat button follows opacity",
			"sessionFollow.desc": "When unified blur is on: whether the New chat button follows the Opacity slider (same as sidebar). Off = keeps its original button color",
