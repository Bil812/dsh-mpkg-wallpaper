window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			// ①(修正) 侧边栏/标题栏局部毛玻璃（方案 E，多轮实测验证）：
			//  - 模糊层 = sidebarCol/header 的 ::before（backdrop-filter blur），
			//    z-index:-1 在内容之下、pointer-events:none 不挡点击。
			//  - 内容容器 hHd-Xa_root / header 直接子元素 z-index:1 → 图标/按钮/文字清晰。
			//  - ::before 不是设置弹窗（fixed）的祖先 → 不困弹窗（实测 w=800 正常）。
			//  - 不加 isolation（会困 stacking context）；display:contents 中间层用
			//    hHd-Xa_root 真实容器提升（实测生效）。
			//  - 只模糊侧边栏/标题栏背后（局部），壁纸其他地方清晰 —— 满足
			//    「不修改整张壁纸、只改遮挡部分」需求；适配悬浮（圆角跟随）。
...
`;
			if (headerBlur && headerBg) {
				...
				// ①(修正) 标题栏雾底**始终**由「标题栏磨砂程度」条（headerBlurAmount）控制，
				// 不随统一虚化（sidebarAlpha）——用户实测「标题栏磨砂程度」在 unify 开时
				// 无效果，故解耦：标题栏独立调节，整屏虚化只管侧边栏。
				const hblurAmt = section.headerBlurAmount !== void 0 ? section.headerBlurAmount : DEFAULT_HEADER_BLUR_AMOUNT;
				const hdrAlpha = Math.max(0, Math.min(100, hblurAmt));
