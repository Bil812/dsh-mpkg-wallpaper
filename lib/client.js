window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			// ①(修正) 侧边栏/标题栏**整块毛玻璃**（方案 F，全部实测通过）：
			//  - 模糊层 = sidebarCol/header 的 ::before，覆盖**整个容器**（inset:0）→
			//    无分离模糊块、无缝隙、适配悬浮圆角。
			//  - z-index:0 模糊层 + 内容容器 z-index:1（hHd-Xa_root / header 直接子元素）
			//    → 图标/按钮/文字在模糊层之上清晰可见（实测 VIS）。
			//  - ::before 不是设置弹窗（fixed, z:1000 overlay 内）的祖先 → 不困弹窗、
			//    弹窗 z:1000 远超内容 z:1 不被盖（实测 w=800 居中、内容正常）。
			//  - 弹窗面板已强制不透明（见 dialog 规则）→ 背后聊天内容不再透出。
			//  - 不加 isolation（会困 stacking context）。适配悬浮（圆角跟随）。
