window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const DEFAULT_ROUND_COMPAT = true; // ②(新) 第三方插件 UI 圆角兼容开关（默认开）
		...
		function applyFromStorage() {
			...
			if (typeof image === "string" && image.indexOf("host:") === 0) {
				...
				const hostUrl = HOST_BASE + "/media" + image.slice(5); // "host:" 是 5 字符
...
${roundCompat ? `/* ②(新) 第三方插件 UI 圆角兼容：插件注入的内容若有"矩形容器"（有背景但无圆角），
   统一补圆角（:where 零优先级 + 无 !important，插件自己的圆角样式优先）。
   命中范围：data-plugin 容器 + 设置页 section 槽（dshmarket 等插件注入处）。 */
:where([data-plugin], [data-slot="settings.section"]) [class*="wrap"],
:where([data-plugin], [data-slot="settings.section"]) [class*="panel"],
:where([data-plugin], [data-slot="settings.section"]) [class*="box"],
:where([data-plugin], [data-slot="settings.section"]) [class*="section"],
:where([data-plugin], [data-slot="settings.section"]) [class*="container"] {
	border-radius: 10px;
}` : ""}
/* ④ 冲突确认弹窗 */
.mpw_mask {
