window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildCss(section) {
			...
			// ①(修正) 毛玻璃**零 z-index 干预**方案（多轮实测确认）：
			//  - 任何对 sidebarCol/header 内容（hHd-Xa_root/titleRow 等）的 z-index 提升
			//    都会破坏设置弹窗层级：弹窗遮罩 z-index:1000，但内容 z-index:1/0 会盖住
			//    弹窗面板（面板 z-index:1）→ 设置界面显示聊天标题/按钮/对话框（用户实测）。
			//  - ::before 的 backdrop-filter 也会让 header 内容与弹窗竞争层级。
			//  - 安全做法：**不给任何侧边栏/标题栏元素加 backdrop-filter、不加 z-index**。
			//    毛玻璃 = **壁纸层 filter blur（--mpw-bg-blur）+ 表面半透明透出**
			//    （sidebarAlpha 控制白雾）。无缝隙、不困弹窗、图标清晰、设置界面干净、
			//    对第三方插件零影响。这是唯一经全部实测不引入新 bug 的方案。
			const frostedLayers = "";
