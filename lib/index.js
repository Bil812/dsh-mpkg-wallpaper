    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          let remote = null, remoteVersion = null;
          try {
          ...
          json(res, 200, { ok: true, local, localVersion: pkgLocal.version, remote, remoteVersion, hasUpdate: !!remote && remote !== local });
    ...
    webServer.register({
      ...
      handler: (req, res) => {
        try {
          ...
          const base = p || (process.platform === 'win32' ? 'C:\\' : '/');
          ...
          const subdirs = readdirSync(base, { withFileTypes: true })
            ...
            .map((d) => d.name);
          json(res, 200, { ok: true, dir: base, subdirs });
        } catch (err) { json(res, 500, { ok: false, error: String(err && err.message || err) }); }
      },
    });
    ...
    webServer.register({
      ...
      handler: async (req, res) => {
        try {
          ...
          customDir = dir;
          const exts = ['.gif', '.png', '.jpg', '.jpeg', '.webp', '.mp4', '.webm', '.mov'];
          // ①(安全) 只收图片/视频；排除 exe/bat/sh/scr 等可执行与脚本（防病毒注入）
          const files = readdirSync(dir)
            .filter((f) => exts.some((e) => f.toLowerCase().endsWith(e)))
            .map((f) => ({ name: f, type: /\\.(mp4|webm|mov)$/i.test(f) ? 'video' : 'image' }));
