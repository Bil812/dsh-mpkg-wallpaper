...
import { createWriteStream, createReadStream, mkdirSync, existsSync, statSync, readFileSync, readdirSync, writeFileSync } from 'node:fs';
 ...
 *  存入 DATA_DIR 下的 JSON，随 DATA_DIR 持久目录。 */
...
function restoreCustomDir() {
 ...
 C:\Users\14675\Desktop\LZY\merge3-stage\theirs-index.js
 ...
 *  ⑰(fork) 更新源指向本 fork（Bil812/dsh-mpkg-wallpaper）——原版 XHR666 仓库会
 *  把我们的 mask/取色/自适应字体等定制全部覆盖掉，绝不能用原版做热更新源。 */
async function fetchRaw(path) {
  const r = await fetch('https://raw.githubusercontent.com/Bil812/dsh-mpkg-wallpaper/main/' + path);
      ...
      handler: async (req, res) => {
        try {
          ...
          try {
            ...
            try {
              const rel = await fetch('https://api.github.com/repos/Bil812/dsh-mpkg-wallpaper/releases/latest');
