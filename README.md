...
**📦 Wallpaper Engine `.mpkg` 解析（浏览器内完成）**
- 直接选择 `.mpkg` 文件 → 浏览器内解析容器（纯客户端，不上传第三方）
- 视频类壁纸自动播放内嵌 mp4 / 视频纹理；场景类壁纸使用作者生成的 `preview.gif` 动态预览
- **多时段自动切换**：壁纸包内含 `preview_night.gif` / `preview_day.gif` 等多时段素材时，按系统当前时间自动选取
- **可调参数（只读展示）**：解析 `project.json → general.properties`，展示壁纸参数供对照壁纸引擎 App

**🌊 整屏虚化（磨砂）体系**
- **统一虚化**：一个条控制整屏朦胧感（0 = 全透明露出壁纸），聊天区 / 新会话按钮可独立选择是否跟随
- **对话框 / 弹层 / 遮罩虚化**：三个独立开关 + 程度条（屏幕中央窗口 / 弹出菜单 / 背景遮罩各管各的）
- 磨砂模糊条（整张壁纸模糊）、面板不透明度、标题栏磨砂程度（独立可调）
...
- **界面虚化**：统一虚化程度、对话框/弹层/遮罩虚化开关 + 程度、Deep diving 背景框、标题栏磨砂/透出壁纸（磨砂程度独立可调）
- **其他**：侧边栏透出壁纸、轻度锐化、第三方 UI 圆角兼容（默认关）、恢复默认
