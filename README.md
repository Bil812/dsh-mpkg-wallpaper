...
## 🍴 本 Fork 修改内容（Bil812 · 基于上游 v3.1.1）

> 这个 fork 在原插件能力之上，重做了「外观主题」体系，让整个 DSH 界面与壁纸融为一体。
> **核心改动：统一全屏遮罩 · 修复不透明度滑条 · 面板颜色自动匹配壁纸/自定义主题色 · 字体颜色自适应 · 蓝色系清理 · 设置页折叠。**
> **已同步上游 v3.1.1 全部功能**：整屏虚化体系（统一虚化 / 对话框 / 设置面板 / 下载确认弹窗 / 弹层 / 遮罩各自独立）、侧边栏磨砂（Aqua 方案）、大文件混合模式（hybrid）、Steam 壁纸发现、自定义壁纸目录、壁纸轮换、检查更新与一键热更新、时钟等。
> **⚠️ 更新源指向本 fork**：插件内「检查更新/一键更新」从 `Bil812/dsh-mpkg-wallpaper` 拉取，**不会**覆盖为本 fork 的定制（mask/取色/字体自适应等）。

### 1. 统一全屏遮罩（全新架构）
- 新增一个**覆盖全界面的半透明遮罩层**（`#mpw-mask`，位于内容底层、其它弹层之下）：侧边栏、标题栏、聊天区、输入框、气泡、菜单、按钮**全部透出同一层雾色**，不再“一块白一块透”。
- 「面板不透明度」滑条重做：**0–100% 实时连续生效**（0 = 完全露出壁纸，100 = 纯遮罩色）。原版在 unifyTint 开启时滑条被接管、只有 0/100 两个观感状态——已修复。

### 2. 面板颜色自动匹配壁纸 / 自定义主题色
- 自动从壁纸抽主色（48×48 canvas 平均色，视频/GIF 每 2s 刷新；跨域图片回退主题色），面板统一上色。
- 新增 **Office 式衍生色调色板**：「自定义主题色（遮罩色）」= 母色 4 档变亮 + 母色 + 4 档变暗共 9 个同色相色块，另附取色器与「恢复自动」。
- 「面板加重」滑条：在全局遮罩上再叠一层，让侧边栏/标题栏等更醒目。

### 3. 字体颜色自适应 + 自定义
- 按遮罩明暗自动生成**高可读文字色**（亮底深字 / 暗底浅字，次级/三级为同系可读灰），不再用低对比灰。
- 「自定义字体颜色」取色器 + 衍生色面板。
- **光标 / 选区 / 焦点描边**改用文字色衍生（不再品牌蓝）；行内 `<code>`、代码块、气泡、按钮、菜单、设置面板等全部走同一套 token 覆写。

### 4. 蓝色系与硬编码底色清理
- `--dsw-alias-brand-primary` 一族品牌蓝 token 覆写为 ink 衍生色 → 工作区图标、会话提交按钮、插件文字不再“同款蓝”。
- 复制悬浮卡片（硬编码 `#2C2C2E`）等深灰元素强制改为主题色。

### 5. 设置页整理
- 分组折叠（背景来源 / 外观 / 界面虚化 / 透出壁纸 / 其他），默认只展开前两组。
...
- **统一全屏遮罩**：一个位于内容底层的半透明覆盖层覆盖整个界面，所有面板透出同一层雾色；「面板不透明度」0–100% 实时生效（0=完全露出壁纸，100=纯遮罩色）。
- **面板颜色自动匹配壁纸 / 自定义主题色**：侧边栏、标题栏、聊天区、输入框、气泡、代码、菜单、按钮等全部取同一遮罩色（自动取壁纸主色，或 Office 式衍生色调色板自定义）。
- **字体颜色自适应**：按遮罩明暗自动生成高可读文字色（亮底深字 / 暗底浅字），支持自定义字体颜色；光标 / 选区 / 焦点描边同步跟随，不再残留品牌蓝。
- **硬编码底色接管**：复制悬浮卡片（#2C2C2E）等硬编码深灰元素强制换为主题色。
- **设置页整理**：分组折叠（背景来源 / 外观 / 界面虚化 / 透出壁纸 / 其他）。
