...
**🌊 整屏虚化（磨砂）体系**
- **统一虚化**：一个条控制整屏壁纸模糊度（0 = 壁纸清晰，拉高 = 越来越模糊），表面白雾厚度统一跟随「不透明度」条
- **对话框 / 弹层 / 遮罩虚化**：三个独立开关 + 程度条（屏幕中央窗口 / 弹出菜单 / 背景遮罩各管各的）
- 磨砂模糊条（统一虚化开启时被接管并提示）、面板不透明度、标题栏磨砂程度（独立可调）
...
*壁纸引擎背景的设置界面。截图之外，外观几乎全部可调：统一虚化（一个条控制整屏壁纸模糊度）、对话框/弹层/遮罩虚化、镜头缩放与平移、侧边栏/标题栏透出壁纸、标题栏磨砂程度、轻度锐化，以及部分壁纸的按时间自动切换。*
...
| 统一虚化 | 一个条控制整屏壁纸模糊度（0=清晰，拉高=更模糊）；表面白雾统一跟随「不透明度」条 |
